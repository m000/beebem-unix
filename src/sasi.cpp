...
void SASIReset(void)
{
    ...
    if (!GetLocation_scsi(pathbuff, 256))
        qERROR("Unable to read SASI disk.");
    //<+
    ...
    for (i = 0; i < 1; ++i) // only one drive allowed under Torch Z80 ?
    {
        ...
        //++
        //	sprintf(buff, "%s/media/scsi/sasi%d.dat", DATA_DIR, i);
        //	printf("%s\n", buff);

        sprintf(buff, "%ssasi%d.dat", pathbuff, i);
        pINFO(dL "Opening/Closing SASI file: '%s'\n", dR, buff);
        //->
        //        if (SASIDisc[i] != NULL)
        //			fclose(SASIDisc[i]);
        //++
...
void SASIExecute(void)
{
    ...
    fprintf(stderr,
            "Unknown Command 0x%02x, Param 1=0x%02x, Param 2=0x%02x, Param 3=0x%02x, Param 4=0x%02x, Param 5=0x%02x, "
            "Phase = %d, PC = 0x%04x\n",
...

{
    ...
    //++
    sprintf(buff, "%s/media/scsi/sasi%d.dat", DATA_DIR, sasi.lun);
    printf("%s\n", buff);
    //<-
