...
static void DoSelects(void)
{
    ...
    if (Selects[0])
        Internal_DriveControlOutputPort |= 0x40;
    if (Selects[1])
        Internal_DriveControlOutputPort |= 0x80;
}; /* DoSelects */
...
static int SkipBadTracks(int Unit, int trackin)
{
    ...
    {
        if (Internal_BadTracks[Unit][0] <= trackin)
            offset++;
        if (Internal_BadTracks[Unit][1] <= trackin)
            offset++;
    }
...
static TrackType *GetTrackPtr(int LogicalTrackID)
{
    ...
    if (LogicalTrackID >= TRACKSPERDRIVE)
...
static SectorType *GetSectorPtr(TrackType *Track, int LogicalSectorID, int FindDeleted)
{
    ...
    for (CurrentSector = 0; CurrentSector < Track->NSectors; CurrentSector++)
        //->    if ((Track->Sectors[CurrentSector].IDField.RecordNum==LogicalSectorID) &&
        //((!Track->Sectors[CurrentSector].Deleted) || (!FindDeleted)))
        //++
        if ((Track->Sectors[CurrentSector].IDField.RecordNum == (unsigned int)LogicalSectorID) &&
            ((!Track->Sectors[CurrentSector].Deleted) || (!FindDeleted)))
            //<-
...
static int ValidateSector(SectorType *SecToVal, int Track, int SecLength)
{
    ...
    //++
    if (SecToVal->IDField.CylinderNum != (unsigned int)Track)
        return (0);
    if (SecToVal->IDField.PhysRecLength != (unsigned int)SecLength)
        return (0);
    //<-
    return (1);
}; /* ValidateSector */
    ...
    if (CommandStatus.ByteWithinSector >= CommandStatus.SectorLength)
    {
        ...
        if (--CommandStatus.SectorsToGo)
        {
            ...
            CommandStatus.CurrentSectorPtr =
                GetSectorPtr(CommandStatus.CurrentTrackPtr, CommandStatus.CurrentSector, 0);
            if (CommandStatus.CurrentSectorPtr == NULL)
            {
                DoErr(0x1e); /* Sector not found */
                return;
...
static void ReadInterrupt(void)
{
    ...
    /*cerr << "ReadInterrupt called - DataReg=0x" << hex << int(DataReg) << dec << "ByteWithinSector=" <<
    ...
    if (CommandStatus.ByteWithinSector >= CommandStatus.SectorLength)
    {
        ...
        {
            CommandStatus.CurrentSector++;
            if (CommandStatus.CurrentSectorPtr =
                    GetSectorPtr(CommandStatus.CurrentTrackPtr, CommandStatus.CurrentSector, 0),
...
static PrimaryCommandLookupType PrimaryCommandLookup[] = {
    ...
    {0x04, 0x3f, 3, DoVarLength_ScanDataAndDeldCommand, NULL,
     "Scan Data & deleted data (Variable Length/Multi-Record)"},
    ...
    {0x17, 0x3f, 3, DoVarLength_ReadDataAndDeldCommand, ReadInterrupt,
     "Read Data & deleted data (Variable Length/Multi-Record)"},
    {0x1b, 0x3f, 3, DoReadIDCommand, ReadIDInterrupt, "ReadID"},
    {0x1e, 0x3f, 2, Do128ByteSR_VerifyDataAndDeldCommand, NULL,
     "Verify Data and Deleted Data (128 byte/single record)"},
    {0x1f, 0x3f, 3, DoVarLength_VerifyDataAndDeldCommand, VerifyInterrupt,
     "Verify Data and Deleted Data (Variable Length/Multi-Record)"},
...
void LoadSimpleDSDiscImage(char *FileName, int DriveNum, int Tracks)
{
    ...
    if (CheckForCatalogue(DiscStore[DriveNum][1][0].Sectors[0].Data, DiscStore[DriveNum][1][0].Sectors[1].Data) == 0)
    {
        //--#ifdef WIN32
        MessageBox(GETHWND,
                   "WARNING - Incorrect disc type selected?\n\n"
...
static void LoadStartupDisc(int DriveNum, char *DiscString)
{
    ...
    if (scanfres = sscanf(DiscString, "%c:%d:%s", &DoubleSided, &Tracks, Name), scanfres != 3)
    {
        //--#ifdef WIN32
        MessageBox(GETHWND,
                   "Incorrect format for BeebDiscLoad, correct format is "
    ...
    else
    {
        ...
        case 'a':
            if ((MachineType == 3) || (!NativeFDC))
                ...
                MessageBox(
                    GETHWND,
                    "The 8271 FDC Cannot load the ADFS disc image specified in the BeebDiscLoad environment variable",
                    "BeebEm", MB_ICONERROR | MB_OK);
            break;

        default:
            //--#ifdef WIN32
            MessageBox(GETHWND,
                       "BeebDiscLoad disc type incorrect, use S for single sided, "
...
void Disc8271_reset(void)
{
    ...
    Internal_BadTracks[0][0] = Internal_BadTracks[0][1] = Internal_BadTracks[1][0] = Internal_BadTracks[1][1] =
        0xff;                      /* 1st subscript is surface 0/1 and second subscript is badtrack 0/1 */
...
void disc8271_dumpstate(void)
{
    ...
    cerr << "  Internal_BadTracks="
         << "(" << int(Internal_BadTracks[0][0]) << "," << int(Internal_BadTracks[0][1]) << ") (";
