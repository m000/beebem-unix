...
unsigned char HiddenDefault[31] = {0, 0, 0, 0,    0,    0,    2,    1, 1,    0,    0xe0, 0x8e, 0,    0,    0,   0,
                                   0, 0, 0, 0xef, 0xff, 0xff, 0x78, 0, 0x17, 0x23, 0x19, 5,    0x0a, 0x2d, 0xa0};
/* End of Computech (&B+) Specific Stuff */
...
int CMOSDefault[64] = {0, 0, 0,    0,    0, 0xc9, 0xff, 0xfe, 0x32,
                       0, 7, 0xc1, 0x1e, 5, 0,    0x58, 0xa2}; // Backup of CMOS Defaults
...
char *BeebMemPtrWithWrap(int a, int n)
{
    ...
    if (MachineType == 3 && a <= EndAddr && Sh_Display > 0 && a < 0x3000)
    {
        if (0x3000 - a < n)
        {
            ...
            if (toCopy > n)
                toCopy = n;
            if (toCopy > 0)
                memcpy(tmpBuf, FSRam + 0x2000 - toCopy, toCopy);
            tmpBufPtr = tmpBuf + toCopy;
            toCopy = n - toCopy;
            if (toCopy > 0)
                memcpy(tmpBufPtr, ShadowRAM + EndAddr - (toCopy - 1), toCopy);
            return (tmpBuf);
        }
        else if (a < 0x1000)
        {
    ...
    if (toCopy > n)
        toCopy = n;
    if (toCopy > 0 && Sh_Display == 0)
        memcpy(tmpBuf, WholeRam + a, toCopy);
    if (toCopy > 0 && Sh_Display > 0)
        memcpy(tmpBuf, ShadowRAM + a, toCopy);
    tmpBufPtr = tmpBuf + toCopy;
    toCopy = n - toCopy;
    if (toCopy > 0 && Sh_Display == 0)
        memcpy(tmpBufPtr, WholeRam + EndAddr - (toCopy - 1), toCopy);
    if (toCopy > 0 && Sh_Display > 0)
        memcpy(tmpBufPtr, ShadowRAM + EndAddr - (toCopy - 1), toCopy);
...
int BeebReadMem(int Address)
{
    ...
    if (MachineType == 1)
    {
        ...
        if (Address == 0xfe3c)
        {
            time(&long_time);
            if (HidAdd == 0)
                return (localtime(&long_time)->tm_sec);
            if (HidAdd == 2)
                return (localtime(&long_time)->tm_min);
            if (HidAdd == 4)
                return (localtime(&long_time)->tm_hour);
            if (HidAdd == 6)
                return ((localtime(&long_time)->tm_wday) + 1);
            if (HidAdd == 7)
                return (localtime(&long_time)->tm_mday);
            if (HidAdd == 8)
                return ((localtime(&long_time)->tm_mon) + 1);
            if (HidAdd == 9)
                return ((localtime(&long_time)->tm_year) - 10);
            if (HidAdd == 0xa)
                return (0x0);
    ...
    if (MachineType == 2)
    {
        if (Address < 0x3000)
            return (WholeRam[Address]);
        if ((Address < 0x8000) && (Sh_Display == 1) && (PrePC >= 0xC000) && (PrePC < 0xE000))
            return (ShadowRAM[Address]);
        if ((Address < 0x8000) && (Sh_Display == 1) && (MemSel == 1) && (PrePC >= 0xA000) && (PrePC < 0xB000))
            return (ShadowRAM[Address]);
        if (Address < 0x8000)
            return (WholeRam[Address]);
...
#ifdef WITH_ECONET
    if (EconetEnabled &&
        ((MachineType != 3 && (Address & ~3) == 0xfe18) || (MachineType == 3 && (Address & ~3) == 0xfe38)))
    {
        if (DebugEnabled)
...
#ifdef WITH_ECONET
    if (EconetEnabled &&
        ((MachineType != 3 && (Address & ~3) == 0xfe20) || (MachineType == 3 && (Address & ~3) == 0xfe3c)))
    {
        if (DebugEnabled)
            DebugDisplayTrace(DEBUG_ECONET, true, "Econet: INTON");
        if (!EconetNMIenabled)
        {                             // was off
            ...
            { // irq pending?
                ...
                if (DebugEnabled)
                    DebugDisplayTrace(DEBUG_ECONET, true, "Econet: delayed NMI asserted");
            }
        }
    }
#endif
...
void BeebWriteMem(int Address, int Value)
{
    ...
    if (MachineType == 0)
    {
        ...
        {
            if (RomWritable[ROMSEL])
                Roms[ROMSEL][Address - 0x8000] = Value;
            return;
        }
    }
    // BBC B End
...
#ifdef WITH_ECONET
    if (EconetEnabled &&
        ((MachineType != 3 && (Address & ~8) == 0xfe18) || (MachineType == 3 && (Address & ~8) == 0xfe38)))
    {
        if (DebugEnabled)
...
void BeebReadRoms(void)
{
    ...
    pINFO(dL "Loading ROM images from '%s':", dR, RomPath);

    RomCfg = fopen(TmpPath, "rt");

    if (RomCfg == NULL)
    {
        pFATAL(dL "roms.cfs [%s] load failed (unable to open file"
                  " - please physically check file).",
               dR, TmpPath);
        exit(1);
    }
    ...
    if (RomCfg != NULL)
    {
        ...
        if (InFile != NULL)
        {
            ...
            {
                pFATAL(dL "OS ROM [%s] load failed (file was too small)..", dR, fullname);
                exit(1);
            }
            else
            {
                ...
                {
                    pINFO(dL "OS ROM [%s] loaded successfully.", dR, RomName);
                }
                else
                {
                    pWARN(dL "OS ROM [%s] did load, but this file is too large!", dR, fullname);
                }
            }
            fclose(InFile);
        }
        ...
        {
            pFATAL(dL "OS ROM [%s] load failed (unable to open"
                      " file)..",
                   dR, fullname);
            exit(1);
        }
        ...
        for (romslot = 15; romslot >= 0; romslot--)
        {
            ...
            if (strncasecmp(RomName, "RAM", 3) == 0)
            {
                RomWritable[romslot] = 1;
                isrom = 0;
            }
            ...
            if (isrom == 1)
            {
                ...
                if (InFile != NULL)
                {
                    if (fread(Roms[romslot], 1, 16384, InFile) < 16384)
                    {
                    ...
                    else
                    {
                        ...
                        if (fread(tmp, 1, 1, InFile) == 0)
                        {
                            pINFO(dL "Paged ROM 0x%X [%s] loaded successfully.", dR, romslot, RomName);
                        }
                        else
                        {
                            pWARN(dL "Paged ROM 0x%X [%s] did load, but this file is too large!", dR, romslot,
                                  fullname);
                        }
                    }
            ...
            {
                pINFO(dL "Paged ROM 0x%X is set as '%s'", dR, romslot, RomName);
            }
        }
        fclose(RomCfg);
    }
...
void BeebMemInit(unsigned char LoadRoms, unsigned char SkipIntegraBConfig)
{
    ...
    //++
    qINFO("Loading CMOS RAM.");

    if (GetLocation_cmos_ram(TmpPath, 256) == NULL)
        qWARN("Can't find cmos.ram, guessing defaults instead!");
    //<-
    ...
    if ((CMDF3 = fopen(TmpPath, "rb")) != NULL)
    {
        ...
        if (CMOSLength == 50)
        {
            for (CMA3 = 0xe; CMA3 < 64; CMA3++)
            {
                ...
                pDEBUG(dL "CMOSRAM[0x%02X]=0x%02X", dR, (int)CMA3, (int)CMOSRAM[CMA3]);
            }
        }
        else
        {
            pERROR(dL "CMOS file %s is not 50 bytes long, guessing defaults instead!", dR, TmpPath);
        }
        fclose(CMDF3);
    }
    else
