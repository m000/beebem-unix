...
static unsigned char CFG_DISABLE_WINDOWS_KEYS[24] = {00, 00, 00,   00,   00, 00, 00,   00,   03, 00, 00, 00,
                                                     00, 00, 0x5B, 0xE0, 00, 00, 0x5C, 0xE0, 00, 00, 00, 00};
...
void BeebWin::ResetBeebSystem(unsigned char NewModelType, unsigned char TubeStatus, unsigned char LoadRoms)
{
    ...
    {
        // 8271 disc
        if ((DiscLoaded[0]) && (CDiscType[0] == 0))
            LoadSimpleDiscImage(CDiscName[0], 0, 0, 80);
        if ((DiscLoaded[0]) && (CDiscType[0] == 1))
            LoadSimpleDSDiscImage(CDiscName[0], 0, 80);
        if ((DiscLoaded[1]) && (CDiscType[1] == 0))
            LoadSimpleDiscImage(CDiscName[1], 1, 0, 80);
        if ((DiscLoaded[1]) && (CDiscType[1] == 1))
            LoadSimpleDSDiscImage(CDiscName[1], 1, 80);
    }
    if (((MachineType != 3) && (!NativeFDC)) || (MachineType == 3))
    {
        // 1770 Disc
        if (DiscLoaded[0])
            Load1770DiscImage(CDiscName[0], 0, CDiscType[0], m_hMenu);
        if (DiscLoaded[1])
            Load1770DiscImage(CDiscName[1], 1, CDiscType[1], m_hMenu);
    }
}
/****************************************************************************/
...
void BeebWin::doLED(int sx, bool on)
{
    ...
    if (sx < 100)
        colbase = LED_COL_BASE; // Red leds for keyboard always
...
BOOL BeebWin::UpdateTiming(void)
{
    ...
    /* Don't do anything if this is the first call or there has
       ...
       wraps. */
    if (m_LastTickCount == 0 || TickCount < m_LastTickCount || (TickCount - m_LastTickCount) > 1000 ||
    ...
    if (TickCount >= m_LastStatsTickCount + 1000)
    {
        ...
        m_ScreenRefreshCount = 0;
        m_RelativeSpeed = ((TotalCycles - m_LastStatsTotalCycles) / 2000.0) / (TickCount - m_LastStatsTickCount);
        m_LastStatsTotalCycles = TotalCycles;
...
{
    static int speeds[] = {
        IDM_FIXEDSPEED100,  IDM_FIXEDSPEED50,   IDM_FIXEDSPEED10,   IDM_FIXEDSPEED5,   IDM_FIXEDSPEED2,
        IDM_FIXEDSPEED1_5,  IDM_FIXEDSPEED1_25, IDM_FIXEDSPEED1_1,  IDM_REALTIME,      IDM_FIXEDSPEED0_9,
        IDM_FIXEDSPEED0_75, IDM_FIXEDSPEED0_5,  IDM_FIXEDSPEED0_25, IDM_FIXEDSPEED0_1, 0};
    int s = 0;
...
void BeebWin::NewTapeImage(char *FileName)
{
     ...
     */
    if (strlen(FileName) < 4 || (strlen(FileName) >= 4 && strcasecmp(FileName + (strlen(FileName) - 4), ".uef")))
        strcat(FileName, ".uef");
...
void BeebWin::NewDiscImage(int Drive)
{
    ...
    if (Save_GTK_File_Selector(FileName) == true)
    {
        ...
        if (FileName[0] == '\0')
        {
            MessageBox(GETHWND, "No name given for file!", "Create floopy disc", MB_OK | MB_ICONERROR);
            return;
        }
        ...
        if (FileName[strlen(FileName) - 1] == '/' || FileName[strlen(FileName) - 1] == '\\')
        {
            MessageBox(GETHWND, "Cannot replace whole directory as disc image!", "Create floopy disc",
                       MB_OK | MB_ICONERROR);
            return;
        }
...
void BeebWin::LoadPreferences()
{
    ...
    *binsize = 1;
    ...
    {
        GetPrivateProfileString(CFG_VIEW_SECTION, CFG_VIEW_DIRECT_ENABLED, "0", CfgValue, sizeof(CfgValue),
                                CFG_FILE_NAME);
    ...
    {
        GetPrivateProfileString(CFG_VIEW_SECTION, CFG_VIEW_BUFFER_IN_VIDEO, "0", CfgValue, sizeof(CfgValue),
                                CFG_FILE_NAME);
    ...
    else
    {
        ...
        GetPrivateProfileString(CFG_VIEW_SECTION, CFG_VIEW_WIN_SIZE, DefValue, CfgValue, sizeof(CfgValue),
                                CFG_FILE_NAME);
    ...
    else
    {
        ...
        GetPrivateProfileString(CFG_VIEW_SECTION, CFG_VIEW_SHOW_FPS, "0", CfgValue, sizeof(CfgValue), CFG_FILE_NAME);
        //<+
    ...
    {
        GetPrivateProfileString(CFG_VIEW_SECTION, CFG_VIEW_MONITOR, "0", CfgValue, sizeof(CfgValue), CFG_FILE_NAME);
    ...
    *binsize = 1;
    ...
    else
    {
        ...
        GetPrivateProfileString(CFG_SPEED_SECTION, CFG_SPEED_TIMING, DefValue, CfgValue, sizeof(CfgValue),
                                CFG_FILE_NAME);
    ...
    {
        GetPrivateProfileString(CFG_SOUND_SECTION, CFG_SOUND_DIRECT_ENABLED, "0", CfgValue, sizeof(CfgValue),
                                CFG_FILE_NAME);
    ...
    else
    {
        ...
        GetPrivateProfileString(CFG_SOUND_SECTION, CFG_SOUND_SAMPLE_RATE, DefValue, CfgValue, sizeof(CfgValue),
                                CFG_FILE_NAME);
    ...
    else
    {
        ...
        GetPrivateProfileString(CFG_SOUND_SECTION, CFG_SOUND_VOLUME, DefValue, CfgValue, sizeof(CfgValue),
                                CFG_FILE_NAME);
    ...
    RegRes =
        SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "ExponentialVolume", &SoundExponentialVolume, binsize);
    if (!RegRes)
    {
        SoundExponentialVolume = 1;
    }
    RegRes =
        SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "TextToSpeechEnabled", &m_TextToSpeechEnabled, binsize);
    if (!RegRes)
    {
    ...
    {
        GetPrivateProfileString(CFG_OPTIONS_SECTION, CFG_OPTIONS_STICKS, "0", CfgValue, sizeof(CfgValue),
                                CFG_FILE_NAME);
    ...
    {
        GetPrivateProfileString(CFG_OPTIONS_SECTION, CFG_OPTIONS_FREEZEINACTIVE, "1", CfgValue, sizeof(CfgValue),
                                CFG_FILE_NAME);
    ...
    {
        GetPrivateProfileString(CFG_OPTIONS_SECTION, CFG_OPTIONS_HIDE_CURSOR, "0", CfgValue, sizeof(CfgValue),
                                CFG_FILE_NAME);
    ...
    else
    {
        ...
        GetPrivateProfileString(CFG_OPTIONS_SECTION, CFG_OPTIONS_KEY_MAPPING, DefValue, CfgValue, sizeof(CfgValue),
                                CFG_FILE_NAME);
    ...
    *binsize = 1;
    ...
    {
        GetPrivateProfileString(CFG_AMX_SECTION, CFG_AMX_ENABLED, "0", CfgValue, sizeof(CfgValue), CFG_FILE_NAME);
    ...
    {
        GetPrivateProfileString(CFG_AMX_SECTION, CFG_AMX_LRFORMIDDLE, "1", CfgValue, sizeof(CfgValue), CFG_FILE_NAME);
    ...
    else
    {
        ...
        GetPrivateProfileString(CFG_AMX_SECTION, CFG_AMX_SIZE, DefValue, CfgValue, sizeof(CfgValue), CFG_FILE_NAME);
    ...
    else
    {
        ...
        GetPrivateProfileString(CFG_AMX_SECTION, CFG_AMX_ADJUST, DefValue, CfgValue, sizeof(CfgValue), CFG_FILE_NAME);
    ...
    {
        GetPrivateProfileString(CFG_PRINTER_SECTION, CFG_PRINTER_ENABLED, "0", CfgValue, sizeof(CfgValue),
                                CFG_FILE_NAME);
    ...
    else
    {
        ...
        GetPrivateProfileString(CFG_PRINTER_SECTION, CFG_PRINTER_PORT, DefValue, CfgValue, sizeof(CfgValue),
                                CFG_FILE_NAME);
    ...
    {
        GetPrivateProfileString(CFG_PRINTER_SECTION, CFG_PRINTER_FILE, "", CfgValue, sizeof(CfgValue), CFG_FILE_NAME);
    ...
    *binsize = 1;
    ...
    RegRes = SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "TeleTextAdapterEnabled", &TeleTextAdapterEnabled,
                                   binsize);
    if (!RegRes)
    {
...
bool BeebWin::TextToSpeechSearch(TextToSpeechSearchDirection dir, TextToSpeechSearchType type)
{
    //--	bool done = false;
    bool found = false;
