...
UINT GetMenuState(HMENU hMenu, UINT uId, UINT uFlags)
{
    ...
    the item, and the high-order byte contains the number of items in the submenu opened by the item.
        ...
        Otherwise, the return value is a mask (Bitwise OR) of the menu flags. Following are the menu flags associated
    with the menu item.
        ...
        MF_MENUBARBREAK	This is the same as the MF_MENUBREAK flag, except for drop-down menus, submenus, and shortcut
    menus, where the new column is separated from the old column by a vertical line. MF_MENUBREAK	The item is placed
    on a new line (for menu bars) or in a new column (for drop-down menus, submenus, and shortcut menus) without
    separating columns. MF_OWNERDRAW	The item is owner-drawn. MF_POPUP	Menu item is a submenu. MF_SEPARATOR
    ...
    Remarks
        ...
        Note  The GetMenuState function has been superseded by the GetMenuItemInfo. You can still use GetMenuState,
    however, if you do not need any of the extended features of GetMenuItemInfo.

        In addition, it is possible to test an item for a flag value of MF_ENABLED, MF_STRING, MF_UNCHECKED, or
    MF_UNHILITE. However, since these values equate to zero you must use an expression to test for them. Flag
    Expression to test for the flag MF_ENABLED	! (Flag&(MF_DISABLED | MF_GRAYED)) MF_STRING	! (Flag&(MF_BITMAP |
    MF_OWNERDRAW)) MF_UNCHECKED	! (Flag&MF_CHECKED) MF_UNHILITE	! (Flag&HILITE)
                              ...
                              DWORD nSize, LPCTSTR lpFileName)
{
    ...
    lpDefault
        [in] A default string. If the lpKeyName key cannot be found in the initialization file, GetPrivateProfileString
    copies the default string to the lpReturnedString buffer. If this parameter is NULL, the default is an empty string,
    "".
    ...
    lpReturnedString buffer to strip any trailing blanks.
            ...
            Windows Me/98/95:  Although lpDefault is declared as a constant parameter, the system strips any trailing
    blanks by inserting a null character into the lpDefault string before copying it to the lpReturnedString buffer.
    */
