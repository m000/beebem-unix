...
struct ECOLAN
{ // what we we need to find a beeb?
...
void EconetReset(void)
{
    ...
    if (EconetEnabled)
    {
        qDEBUG("Econet hardware enabled.");
    }
    else
    {
        qDEBUG("Econet hardware disabled.");
    }
    //<-
    ...
    {
        sprintf(info, "Failed to open listening socket for econet.");
        pERROR(dL "%s", dR);
        EconetError(info);
        return;
    }
    ...
    if (EconetStationNumber != 0)
    {
        ...
        if (EconetListenPort != 0)
        {
            ...
            //--				sprintf(info, "Econet: Failed to bind to port %d (error %ld)", EconetListenPort,
            //WSAGetLastError());
            //--				EconetError(info);
            //--				closesocket(ListenSocket);
            //--				WSACleanup();
            ...
            {
                sprintf(info, "Failed to bind to port %d.", EconetListenPort);
                EconetError(info);
                close(ListenSocket);
                return;
            }
        ...
        else
        {
            ...
            //--			return;
            //++
            sprintf(info, "Failed to find station %d in econet.cfg", EconetStationNumber);
            EconetError(info);
            return;
            //<-
        }
        ...
        if (gethostname(localhost, 256) != -1 && (hent = gethostbyname(localhost)) != NULL)
        {
            ...
            for (unsigned int i = 0; i < networkp && EconetStationNumber == 0; ++i)
            {
                ...
                for (int a = 0; hent->h_addr_list[a] != NULL && EconetStationNumber == 0; ++a)
                {
                    ...
                    //++
                    if (network[i].inet_addr == inet_addr("127.0.0.1") || network[i].inet_addr == localaddr.s_addr)
                    {
                        //<-
    ...
    {
        EconetError("Econet: Failed to open sending socket.");
        close(ListenSocket);
        return;
    }
    //<-
...
void ReadNetwork(void)
{
    ...
    //++
    if (GetLocation_econet_cfg(TmpPath, 256) == NULL)
        qERROR("Unable to determine econet.cfg filepath.");
    //<-
    ...
    if (EcoCfg == NULL)
    {
        ...
        //--		EconetError(info);
        //++
        sprintf(info, "Econet: Failed to open econet configuration file: %s", TmpPath);
        EconetError(info);
        //<-
        networkp = 0;
        network[0].station = 0;
    ...
    else
    {
        ...
        do
        {
            ...
            if (EcoName[0] != '#')
            {
                ...
                //--				}
                //++
                pDEBUG(dL "ConfigFile Net %i Stn %i IP %08x Port %i", dR, network[networkp].network,
                       network[networkp].station, network[networkp].inet_addr, network[networkp].port);
                //<-
                if (p == 4)
...
unsigned char Read_Econet_Station(void)
{
    ...
    //--	}
    //++
    pDEBUG(dL "Econet read station %02X", dR, (int)EconetStationNumber);
    //<-
...
unsigned char ReadEconetRegister(unsigned char Register)
{
    ...
    //--	}
    //++
    pDEBUG(dL "Read econet ADLC %02X", dR, (int)Register);
    debugADLCprint();
    //<-
    if (Register == 0)
    {
    ...
    if (Register > 1)
    {
        if (((ADLC.control1 & 64) == 0) && ADLC.rxfptr)
        {   // rxreset not set and someting in fifo
            ...
            //--			}
            //++
            pDEBUG(dL "Econet returned FIFO:  %02X", dR, (int)ADLC.rxfifo[ADLC.rxfptr - 1]);
            debugADLCprint();
            //<-
            if (ADLC.rxfptr)
            {
...
void WriteEconetRegister(unsigned char Register, unsigned char Value)
{
    ...
    //--	}
    //++
    pDEBUG(dL "Econet write ADLC  %02X = %02X", dR, (int)Register, (int)Value);
    //<-
    ...
    else if (Register == 1 && !(ADLC.control1 & 1))
    { // adr 01 & AC=0
        ADLC.control2 = Value;
    }
    else if (Register == 1 && (ADLC.control1 & 1))
    ...
    else if (Register == 3 && (ADLC.control1 & 1))
    { // adr 03 & AC=1
        ADLC.control4 = Value;
    }
    else if (Register == 2 || Register == 3)
    { // adr 02 or adr 03 & AC=0
        ...
        if ((ADLC.control1 & 128) == 0)
        {
            ...
            if (Register == 3)
                ADLC.control2 |= 16; // set txlast control flag ourself
        }
    }
...
{ // return NMI status
    ...
    if ((ADLC.control2 & 64) || (ADLC.control1 & 128))
    {                          // or txreset
        ...
        { // tx reset off
            ...
            {   // there is data is in tx fifo
                ...
                {   // TxLast set
                    ...
                    //--					}
                    //++
                    pDEBUG(dL "TXLast set - send packet to %02x %02x.", dR, (unsigned int)(Econettxbuff[1]),
                    ...
                    do
                    {
                        ...
                        if (network[i].station != EconetStationNumber)
                        {
                            ...
                            //->							if (DebugEnabled) {
                            //--								sprintf(info, "Econet: TXLast set - Send %d byte packet to %02x %02x
                            //(%08X :%u)",
                            //--										EconetTxWritePointer,
                            ...
                            pDEBUG(dL "TXLast set - Send %d byte packet to"
                                      " %02x %02x (%08X :%u)",
                                   dR, EconetTxWritePointer, (unsigned int)(Econettxbuff[1]),
                                   (unsigned int)Econettxbuff[0], (unsigned int)network[i].inet_addr,
                                   (unsigned int)network[i].port);

                            sprintf(info, "Packet data:");
                            for (unsigned int x = 0; x < EconetTxWritePointer; ++x)
                            {
                                sprintf(info + strlen(info), " %02X", Econettxbuff[x]);
                            }
                            pDEBUG(dL "%s", dR, info);
                            //<-
                            ...
                            //--							}
                            //++
                            if (sendto(SendSocket, (char *)Econettxbuff, EconetTxWritePointer, 0,
                                       (struct sockaddr *)&RecvAddr, sizeof(RecvAddr)) == -1)
                            {
                                sprintf(info,
                                        "Econet: Failed to send packet to"
                                        " %02x %02x (%08X :%u)",
                                        (unsigned int)(Econettxbuff[1]), (unsigned int)Econettxbuff[0],
                    ...
                    //->					if (DebugEnabled) DebugDisplayTrace(DEBUG_ECONET, true, "Econet: FlagFill
                    //set");
                    //++
                    qDEBUG("FlagFill set.");
                    //<-
                    ...
                    if (RetVal > 0)
                    {
                        ...
                        if (RetVal > 0)
                        {
                            ...
                            if (Econetrxbuff[0] == EconetStationNumber)
                            {
                                ...
                                FlagFillActive = false;
                                //--								if (DebugEnabled) DebugDisplayTrace(DEBUG_ECONET,
                                //true,
                            ...
                            else
                            {
                                ...
                                //--								if (DebugEnabled) DebugDisplayTrace(DEBUG_ECONET,
                                //true,
                                //--													"Econet: FlagFill set - other station
                                //comms");
                                //+>
    ...
    if (ReceiverSocketsOpen && (ADLC.control2 & 128))
    { // clock + RTS
        ADLC.cts = FALSE;
        ADLC.status1 &= ~16;
    }
    ...
    {   // probably not needed
        ...
        //--		}
        //++
        pDEBUG(dL "TX Underrun - TXfptr %02x", dR, (unsigned int)ADLC.txfptr);
        //<-
        ADLC.status1 |= 32;
        ADLC.txfptr = 4;
    }
    ...
    if (!(ADLC.control1 & 64))
    { // not rxreset
        if (ADLC.rxfptr && (ADLC.rxap & (powers[ADLC.rxfptr - 1])))
        { // ap bits set on fifo
            ADLC.status2 |= 1;
        }
        else
    ...
    if (ADLC.control2 & 1)
    {
        if ((ADLC.sr2pse <= 1) && (ADLC.status2 & 0x7A))
        { // ERR, FV, DCD, OVRN, ABT
            ADLC.sr2pse = 1;
            ADLC.status2 &= ~0x85;
        }
        else if ((ADLC.sr2pse <= 2) && (ADLC.status2 & 0x04))
        { // Idle
            ADLC.sr2pse = 2;
            ADLC.status2 &= ~0x81;
        }
        else if ((ADLC.sr2pse <= 3) && (ADLC.status2 & 0x01))
        { // AP
            ADLC.sr2pse = 3;
            ADLC.status2 &= ~0x80;
        }
        else if (ADLC.status2 & 0x80)
        { // RDA
            ADLC.sr2pse = 4;
            ADLC.status2 &= ~0x02;
        }
    ...
    if (sr2psetemp != ADLC.sr2pse)
        pDEBUG(dL "ADLC: PSE SR2Rx priority changed to %d.", dR, ADLC.sr2pse);
    //<-

    // Do we need to flag an interrupt?
    if (ADLC.status1 != ADLCtemp.status1 || ADLC.status2 != ADLCtemp.status2)
    { // something changed
        ...
        else if (!(ADLC.status2 & sr1b2cause))
        { // cause has gone
            ADLC.status1 &= ~2;
            sr1b2cause = 0;
        }
        ...
        { // something got set
            ...
            //--			}
            //++
            pDEBUG(dL "ADLC: Status1 bit got set %02x, interrupt", dR, (int)tempcause);
            //<-
        }
        ...
        {                                 // something went off
            ...
            else
            {
                ...
                {
                    interruptnow = TRUE;
                    //->					DebugDisplayTrace(DEBUG_ECONET, true, "ADLC: S1 flags still set,
                    //interrupt");
                    //++
...
void debugADLCprint(void)
{
    ...
    sprintf(info, "ADLC: Ctrl:%02X %02X %02X %02X St:%02X %02X TXFptr:%02x rx:%02x FF:%d IRQc:%02x SR2c:%02x PC:%04x  ",
            (int)ADLC.control1, (int)ADLC.control2, (int)ADLC.control3, (int)ADLC.control4, (int)ADLC.status1,
            (int)ADLC.status2, (int)ADLC.txfptr, (int)ADLC.rxfptr, FlagFillActive ? 1 : 0, (int)irqcause,
            (int)sr1b2cause, (int)ProgramCounter);
    //->	DebugDisplayTrace(DEBUG_ECONET, true, info);
    pDEBUG(dL "%s", dR, info);
    //<-
}
