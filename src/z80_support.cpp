...

{
    ...
    sprintf(str + strlen(str), ":%02X,%02X,%02X,%02X", ReadZ80Mem(pc), ReadZ80Mem(pc + 1), ReadZ80Mem(pc + 2),
            ReadZ80Mem(pc + 3));
}
...

{
    ...
    sprintf(str + strlen(str), ":%02X,%02X,%02X,%02X\n", ReadZ80Mem(sp), ReadZ80Mem(sp + 1), ReadZ80Mem(sp + 2),
            ReadZ80Mem(sp + 3));
}
...

{
    ...
    else
    {
        ...
        if ((addr == 0x06) || (addr == 0x02))
        {
            ...
            tmp = 0x00;
            if (value & 128)
                tmp |= 2; // Tube data available
            if (value & 64)
                tmp |= 128; // Tube not full
            value = tmp;
        }
