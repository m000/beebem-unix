...
int DebugDisassembleInstruction(int addr, bool host, char *opstr)
{
    ...
    case 2:
        ...
        break;
    case 3:
        sprintf(s, "%02X %02X %02X  ", DebugReadMem(addr, host), DebugReadMem(addr + 1, host),
    ...
    while (count > 0 && addr <= 0xffff)
    {
        if ((TorchTube || AcornZ80) && !host)
        {
            ...
            case 3:
                ...
                break;
            case 4:
                sprintf(s, "%02X %02X %02X %02X  ", DebugReadMem(addr, host), DebugReadMem(addr + 1, host),
                        DebugReadMem(addr + 2, host), DebugReadMem(addr + 3, host));
                break;
            }
