...
int main(int argc, char *argv[])
{
    ...
    do
    {
        ...
        if (showing_menu != 1)
        {
             ...
             */
            if (mainWin->CursorShouldBeHidden() && SDL_ShowCursor(SDL_QUERY) == SDL_ENABLE &&
            ...
            while (SDL_PollEvent(&event))
                ...
                case SDL_MOUSEMOTION:
                    ...
                    if (mainWin)
                    {
                        ...
                        mainWin->ScaleMousestick((unsigned int)event.motion.x, (unsigned int)event.motion.y);
                        mainWin->SetAMXPosition((unsigned int)event.motion.x, (unsigned int)event.motion.y);
                ...
                case SDL_KEYUP: {
                    ...
                    if (event.key.keysym.sym == SDLK_F12 || event.key.keysym.sym == SDLK_F11 ||
