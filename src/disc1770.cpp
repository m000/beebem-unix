...
unsigned char Read1770Register(unsigned char Register)
{
    ...
    if ((FDCommand < 6) && (FDCommand != 0))
        Status ^= 2; // Fool anything reading the
...
void Poll1770(int NCycles)
{
    ...
    if ((Status & 1) && (NMILock == 0))
    {
        ...
        if ((FDCommand == 7) && (DWriteable[CurrentDrive] == 1))
        { // Write
            LoadingCycles -= NCycles;
            if (LoadingCycles > 0)
                return;
        ...
        if ((FDCommand >= 8) && (*CDiscOpen == 1) && (FDCommand <= 10))
        { // Read/Write Prepare
            SetStatus(0);
            ResetStatus(5);
            ResetStatus(6);
            ...
            LoadingCycles = 45;
            fseek(CurrentDisc,
                  DiscStrt[CurrentDrive] + (DiscStep[CurrentDrive] * Track) + (Sector * SecSize[CurrentDrive]),
                  SEEK_SET);
        }
    ...
    if ((FDCommand == 23) && (DWriteable[CurrentDrive] == 1))
    { // Write Track
        LoadingCycles -= NCycles;
        if (LoadingCycles > 0)
            return;
    ...
    if ((FDCommand >= 20) && (*CDiscOpen == 1) && (FDCommand <= 21))
    { // Read/Write Track Prepare
        SetStatus(0);
        ResetStatus(5);
        ResetStatus(6);
        ResetStatus(2);
    ...
    if (FDCommand == 10)
    {
        ...
        NMIStatus |= 1 << nmi_floppy;
        FDCommand = 12;
        LoadingCycles = 4000000; // Spin-down delay
        return;
    }
    if (FDCommand == 11)
    {
    ...
    { // Confusion spin
        LoadingCycles -= NCycles;
        if ((LoadingCycles < 2000) && (!(Status & 32)))
            SetStatus(5); // Compelte spin-up, but continuing whirring
        if (LoadingCycles <= 0)
            FDCommand = 10;
    ...
    { // Read Address - just 6 bytes
        ...
        if ((Status & 2) == 0)
        {
            ...
            if (ByteCount == 3)
            {
                if (DiscType[CurrentDrive] == 0)
                    Data = 1; // 256
                if (DiscType[CurrentDrive] == 1)
                    Data = 1; // 256
                if (DiscType[CurrentDrive] == 2)
                    Data = 1; // 256
                if (DiscType[CurrentDrive] == 3)
                    Data = 3; // 1024
                if (DiscType[CurrentDrive] == 4)
                    Data = 2; // 512
            }
            if (ByteCount == 2)
                Data = 0;
