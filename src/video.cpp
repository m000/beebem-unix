...
unsigned char CRTC_SyncWidth =
    0x28; /* R3 - top 4 bits are Vertical (in scan lines) and bottom 4 are horizontal in characters */
...
unsigned char CRTC_InterlaceAndDelay =
    0; /* R8 - 0,1 are interlace modes, 4,5 display blanking delay, 6,7 cursor blanking delay */
...
typedef struct
{
    ...
    int CharLine; /* 6845 counts in characters vertically - 0 at the top , incs by 1 - -1 means we are in the bit before
                     the actual display starts */
...
static int Mode7DoubleHeightFlags[80]; /* Pessimistic size for this flags - if 1 then corresponding character on NEXT
                                          line is top half */
...
static void BuildMode7Font(void)
{
    ...
    for (m7cc = 0; m7cc < 96; m7cc++)
    {
        ...
        if (!(m7cc & 32))
        {
            ...
            EM7Font[2][m7cc][3] = row1;
            EM7Font[2][m7cc][4] = EM7Font[2][m7cc][5] = 0;
            ...
            EM7Font[2][m7cc][17] = row3;
            EM7Font[2][m7cc][18] = EM7Font[2][m7cc][19] = 0;
...
static void DoFastTable16(void)
{
    ...
    {
        bplvopen = ((beebpixvl & 8) ? 128 : 0) | ((beebpixvl & 4) ? 32 : 0) | ((beebpixvl & 2) ? 8 : 0) |
                   ((beebpixvl & 1) ? 2 : 0);
        for (beebpixvr = 0; beebpixvr < 16; beebpixvr++)
        {
            bplvtotal = bplvopen | ((beebpixvr & 8) ? 64 : 0) | ((beebpixvr & 4) ? 16 : 0) | ((beebpixvr & 2) ? 4 : 0) |
                        ((beebpixvr & 1) ? 1 : 0);
            ...
            // [HERE]
            FastTable[bplvtotal].data[0] = FastTable[bplvtotal].data[1] = FastTable[bplvtotal].data[2] =
            ...
            // [HERE]
            FastTable[bplvtotal].data[4] = FastTable[bplvtotal].data[5] = FastTable[bplvtotal].data[6] =
...
static void DoFastTable4(void)
{
    ...
    {
        pentry = ((beebpixv & 128) ? 8 : 0) | ((beebpixv & 32) ? 4 : 0) | ((beebpixv & 8) ? 2 : 0) |
                 ((beebpixv & 2) ? 1 : 0);
        ...
        //
        pentry =
            ((beebpixv & 64) ? 8 : 0) | ((beebpixv & 16) ? 4 : 0) | ((beebpixv & 4) ? 2 : 0) | ((beebpixv & 1) ? 1 : 0);
        tmp = VideoULA_Palette[pentry];
        if (tmp > 7)
        {
            tmp &= 7;
        ...
        //
        pentry = ((beebpixv & 32) ? 8 : 0) | ((beebpixv & 8) ? 4 : 0) | ((beebpixv & 2) ? 2 : 0) | 1;
        tmp = VideoULA_Palette[pentry];
        if (tmp > 7)
        {
            tmp &= 7;
        ...
        //
        pentry = ((beebpixv & 16) ? 8 : 0) | ((beebpixv & 4) ? 4 : 0) | ((beebpixv & 1) ? 2 : 0) | 1;
        tmp = VideoULA_Palette[pentry];
        if (tmp > 7)
        {
            tmp &= 7;
...
static void DoFastTable4XStep4(void)
{
    ...
    for (beebpixv = 0; beebpixv < 256; beebpixv++)
    {
        ...
        // [HERE]
        FastTableDWidth[beebpixv].data[0] = FastTableDWidth[beebpixv].data[1] = FastTableDWidth[beebpixv].data[2] =
        ...
        // [HERE]
        FastTableDWidth[beebpixv].data[4] = FastTableDWidth[beebpixv].data[5] = FastTableDWidth[beebpixv].data[6] =
        ...
        // [HERE]
        FastTableDWidth[beebpixv].data[8] = FastTableDWidth[beebpixv].data[9] = FastTableDWidth[beebpixv].data[10] =
        ...
        // [HERE]
        FastTableDWidth[beebpixv].data[12] = FastTableDWidth[beebpixv].data[13] = FastTableDWidth[beebpixv].data[14] =
...
static void DoFastTable2(void)
{
    ...
    for (beebpixv = 0; beebpixv < 256; beebpixv++)
    {
        ...
        for (pix = 0; pix < 8; pix++)
        {
            pentry = ((beebpixvt & 128) ? 8 : 0) | ((beebpixvt & 32) ? 4 : 0) | ((beebpixvt & 8) ? 2 : 0) |
                     ((beebpixvt & 2) ? 1 : 0);
            beebpixvt <<= 1;
            beebpixvt |= 1;
            tmp = VideoULA_Palette[pentry];
...
static void DoFastTable(void)
{
    ...
    {
        LineRoutine =
            (VideoULA_ControlReg & 0x10) ? LowLevelDoScanLineNarrowNot4Bytes : LowLevelDoScanLineWideNot4Bytes;
    };
...
static void VideoStartOfFrame(void)
{
    ...
    if (InterlaceFrame)
    {
        IncTrigger((IL_Multiplier * (CRTC_HorizontalTotal + 1) * ((VideoULA_ControlReg & 16) ? 1 : 2)),
                   VideoTriggerCount); /* Number of 2MHz cycles until another scanline needs doing */
    }
    else
    {
...
static void LowLevelDoScanLineNarrow()
{
    ...
    {
        unsigned char *vidPtr2 = (unsigned char *)mainWin->GetLinePtr(VideoState.PixmapLine);
...
static void LowLevelDoScanLineNarrowNot4Bytes()
{
    ...
    else
    {
        ...
        unsigned char *vidPtr2 = (unsigned char *)mainWin->GetLinePtr16(VideoState.PixmapLine);
        CurrentPtr = (unsigned char *)VideoState.DataPtr + VideoState.InCharLineUp;
        for (; BytesToGo; CurrentPtr += 8, BytesToGo--)
        {
...
static void DoMode7Row(void)
{
    ...
    unsigned int CurrentCol[20] = {0xffffff, 0xffffff, 0xffffff, 0xffffff, 0xffffff, 0xffffff, 0xffffff,
    ...
    for (CurrentChar = 0; CurrentChar < CRTC_HorizontalDisplayed; CurrentChar++)
    {
        ...
        if (byte < 32)
            byte += 128; // fix for naughty programs that use 7-bit control codes - Richard Gellman
        if ((byte & 32) && (Graphics))
            HoldGraphChar = byte;
        ...
        if (!DoubleHeight)
        {
                ...
                if ((tmp == 0) || (tmp == 255))
                {
                    ...
                    else
                    {
                        if (CurrentLen[CurrentScanLine])
                            mainWin->doHorizLine(CurrentCol[CurrentScanLine], VideoState.PixmapLine + CurrentScanLine,
                                                 CurrentStartX[CurrentScanLine], CurrentLen[CurrentScanLine]);
            ...
            Mode7DoubleHeightFlags[CurrentChar] =
                1; /* Not double height - so if the next line is double height it will be top half */
        }
        else
        {
            int ActualScanLine;
    ...
    if (VideoState.IsTeletext)
    {
        ...
        if (!FrameNum)
        {
            ...
            for (l = 0; l < 20 && VideoState.PixmapLine + l < 512; ++l)
                ...
                //++
                mainWin->doHorizLine(0, VideoState.PixmapLine + l, -72, 800);
            //+<
        }
    ...
    else
    {
        ...
        if (VideoState.CharLine != -1)
        {
            if (VideoState.CharLine < CRTC_VerticalDisplayed)
            {
                ...
                {
                    ova = VideoState.Addr * 8;
                    ovn = CRTC_HorizontalDisplayed * 8;
...
void CRTCWrite(int Address, int Value)
{
    ...
    if (Address & 1)
    {
        ...
        //		fprintf(crtclog,"%d (%02X) Written to register %d from
...
int CRTCRead(int Address)
{
    ...
    {
        return (0); /* Rockwell part has bits 5,6,7 used - bit 6 is set when LPEN is received, bit 5 when in vertical
                       retrace */
    }
...
void DebugVideoState()
{
    ...
    sprintf(info, "CRTC: HTot=%02X HDis=%02X HSyn=%02X SWid=%02X VTot=%02X VAdj=%02X VDis=%02X VSyn=%02X",
            (int)CRTC_HorizontalTotal, (int)CRTC_HorizontalDisplayed, (int)CRTC_HorizontalSyncPos, (int)CRTC_SyncWidth,
            (int)CRTC_VerticalTotal, (int)CRTC_VerticalTotalAdjust, (int)CRTC_VerticalDisplayed,
    ...
    sprintf(info, "CRTC: IntD=%02X SLCh=%02X CurS=%02X CurE=%02X ScrS=%02X%02X CurP=%02X%02X VidULA=%02X",
            (int)CRTC_InterlaceAndDelay, (int)CRTC_ScanLinesPerChar, (int)CRTC_CursorStart, (int)CRTC_CursorEnd,
            (int)CRTC_ScreenStartHigh, (int)CRTC_ScreenStartLow, (int)CRTC_CursorPosHigh, (int)CRTC_CursorPosLow,
...
void VideoGetText(char *text, int line)
{
    ...
    char *dataPtr =
        BeebMemPtrWithWrapMo7(VideoState.StartAddr + line * CRTC_HorizontalDisplayed, CRTC_HorizontalDisplayed);
