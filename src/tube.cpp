...
//->
//## 'NMILock' is declared global in 6502core.cpp and external in 6502core.h.
//## Gcc gives me an error when it's redeclared here as static.  So I need to
//## change something.
//##
//## Hmm, what to do??
//##
//## If NMILock blocks the NMI until an RTI then I'd assume that, that's
//## specific to this CPU and not the hosts. But I know nothing about the tube.
//##
//## So I've changed the name to 'tube_NMILock' throughout this file.
//##
//## If I'm wrong then please replace all occurrences of tube_NMILock with
//## NMILock.
//##
...
#define TUBEWRITEMEM_FAST(Address, Value)                                                                              \
    if (Address < 0xfef8)                                                                                              \
        TubeRam[Address] = Value;                                                                                      \
    else                                                                                                               \
        TubeWriteMem(Address, Value);
#define TUBEWRITEMEM_DIRECT(Address, Value) TubeRam[Address] = Value;
#define TUBEFASTWRITE(addr, val)                                                                                       \
    tmpaddr = addr;                                                                                                    \
    if (tmpaddr < 0xfef8)                                                                                              \
        TUBEWRITEMEM_DIRECT(tmpaddr, val) else TubeWriteMem(tmpaddr, val);
...
{
    if ((R1Status & TubeM) && !(R1Status & TubeV) && ((R3HPPtr > 0) || (R3PHPtr == 0)))
        TubeNMIStatus |= (1 << R3);
    else if ((R1Status & TubeM) && (R1Status & TubeV) && ((R3HPPtr > 1) || (R3PHPtr == 0)))
        TubeNMIStatus |= (1 << R3);
    else
        TubeNMIStatus &= ~(1 << R3);
}
...
unsigned char ReadTorchTubeFromHostSide(unsigned char IOAddr)
{
    ...
    case 0x0d:
        ...
        if (R1HStatus & TubeDataAv)
            TmpData |= 0x02;
        if (R1HStatus & TubeNotFull)
            TmpData |= 0x10;
...
INLINE void Carried()
{
    ...
    if (((CurrentInstruction & 0xf) == 0x1 || (CurrentInstruction & 0xf) == 0x9 || (CurrentInstruction & 0xf) == 0xd) &&
        (CurrentInstruction & 0xf0) != 0x90)
    {
        TubeCycles++;
    }
    else if (CurrentInstruction == 0xBC || CurrentInstruction == 0xBE)
    {
        TubeCycles++;
    }
}
...
INLINE static void ADCInstrHandler(int16 operand)
{
    ...
    if (!GETDFLAG)
    {
        ...
        SetPSR(FlagC | FlagZ | FlagV | FlagN, (TmpResultC & 256) > 0, Accumulator == 0, 0, 0, 0,
               ((Accumulator & 128) > 0) ^ (TmpResultV < 0), (Accumulator & 128));
    }
    else
    {
...
INLINE static void BadInstrHandler(int opcode)
{
    if (!IgnoreIllegalInstructions)
    {
        ...
        char errstr[250];
        sprintf(errstr,
                "Unsupported 65C02 instruction 0x%02X at 0x%04X\n"
                "  OK - instruction will be skipped\n"
                "  Cancel - dump memory and exit",
                opcode, TubeProgramCounter - 1);
...
void Reset65C02(void)
{
    ...
    //++
    //	strcpy(TubeRomName,RomPath);
    //	strcat(TubeRomName,"/other/6502tube.rom");

    qINFO("Loading tube ROM.");
    GetLocation_tube(TubeRomName, 256);
    //<-
    ...
    {
        pERROR(dL "Unable to load tube ROM: '%s'.", dR, TubeRomName);
    }
    //<--
...
void Exec65C02Instruction(void)
{
    ...
    case 0x74:
        if (TubeMachineType == 3)
        {
            ...
            TubeProgramCounter += 1; /* STZ Zpg,X */
        break;
    case 0x75:
    ...
    if ((CurrentInstruction == 0x10) || (CurrentInstruction == 0x30) || (CurrentInstruction == 0x50) ||
        (CurrentInstruction == 0x70) || (CurrentInstruction == 0x80) || (CurrentInstruction == 0x90) ||
        (CurrentInstruction == 0xb0) || (CurrentInstruction == 0xd0) || (CurrentInstruction == 0xf0))
    {
        if (Branched)
        {
            TubeCycles++;
...
void DebugTubeState(void)
{
    ...
    sprintf(info, "HostTube: R1=%02X R2=%02X R3=%02X R4=%02X R1n=%02X R3n=%02X", (int)R1HStatus | R1Status,
            (int)R2HStatus, (int)R3HStatus, (int)R4HStatus, (int)R1PHPtr, (int)R3PHPtr);
    DebugDisplayInfo(info);

    sprintf(info, "ParaTube: R1=%02X R2=%02X R3=%02X R4=%02X R3n=%02X", (int)R1PStatus | R1Status, (int)R2PStatus,
            (int)R3PStatus, (int)R4PStatus, (int)R3HPPtr);
    DebugDisplayInfo(info);
}
