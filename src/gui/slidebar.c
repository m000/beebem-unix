...
static float GetPositionAsFloat(EG_Widget *widget_ptr)
{
    ...
    return (float)slidebar_ptr->position / (float)(slidebar_ptr->virtual_length - CalcVisibleLength(widget_ptr));
}
...
static void Calc_Slider_Area(EG_Widget *widget_ptr)
{
    ...
    case EG_SlideBar_Vertical:
        ...
        slidebar_ptr->slider_area.h = (float)slidebar_ptr->slider_area.h *
                                      ((float)CalcVisibleLength(widget_ptr) / (float)slidebar_ptr->virtual_length);
        if (slidebar_ptr->slider_area.h < 30)
            slidebar_ptr->slider_area.h = 30;

        /* Calc position of slider.
         */
        slidebar_ptr->slider_area.y +=
            (slidebar_ptr->background_area.h - slidebar_ptr->slider_area.h) * GetPositionAsFloat(widget_ptr);
        break;
    ...
    case EG_SlideBar_Horizontal:
        ...
        /* Calc position of slider.
         */
        slidebar_ptr->slider_area.x +=
            (slidebar_ptr->background_area.w - slidebar_ptr->slider_area.w) * GetPositionAsFloat(widget_ptr);
        break;
    }
}
...
static void Move_Slider(EG_Widget *widget_ptr)
{
    ...
    case EG_SlideBar_Vertical:
         ...
         */
        new_slider_position =
            (float)(slidebar_ptr->slider_area.y - slidebar_ptr->background_area.y) + slidebar_ptr->mouse_y;
        ...
        /* Set position to it.
         */
        slidebar_ptr->position = (((float)(slidebar_ptr->virtual_length - CalcVisibleLength(widget_ptr))) *
    ...
    case EG_SlideBar_Horizontal:
         ...
         */
        new_slider_position =
            (float)(slidebar_ptr->slider_area.x - slidebar_ptr->background_area.x) + slidebar_ptr->mouse_x;
...
static EG_BOOL Callback_SDL_Event(EG_Widget *widget_ptr, SDL_Event *event_ptr)
{
     ...
     * false.
     */
    if (EG_Shared_GetEventDetails(widget_ptr, event_ptr, (void *)&slidebar_ptr, &window_ptr, &mouse_over_widget) !=
        EG_TRUE)
        return (return_value);
    ...
    if (mouse_is_over == OVER_TROFF)
    {
        ...
        {
        case EG_SlideBar_Vertical:
            if (event_ptr->motion.y < slidebar_ptr->slider_area.y + EG_Window_GetYDisplacement(window_ptr))
        ...
        case EG_SlideBar_Horizontal:
            if (event_ptr->motion.x < slidebar_ptr->slider_area.x + EG_Window_GetXDisplacement(window_ptr))
                mouse_is_over = OVER_TROFF_AND_ABOVE_SLIDER;
            else
                mouse_is_over = OVER_TROFF_AND_BELOW_SLIDER;
            break;
     ...
     * Yes,
     */
    if (event_ptr->type == SDL_MOUSEBUTTONDOWN || event_ptr->type == SDL_MOUSEBUTTONUP)
    {
        /* Is mouse button 1 physically depressed?
         * Yes,
         */
        if (event_ptr->button.state == SDL_PRESSED && event_ptr->button.button == SDL_BUTTON_LEFT)
        {

            /* Is mouse pointer over button1 area?
             * Yes, does 'button1_depressed' equal false?
             */
            if (mouse_is_over == OVER_BUTTON1 && slidebar_ptr->button1_depressed == EG_FALSE)
            {
             ...
             */
            if (mouse_is_over == OVER_BUTTON2 && slidebar_ptr->button2_depressed == EG_FALSE)
            {
    ...
    if (event_ptr->type == SDL_MOUSEBUTTONDOWN || event_ptr->type == SDL_MOUSEBUTTONUP)
    {
        ...
        if (event_ptr->button.state == SDL_PRESSED && event_ptr->button.button == SDL_BUTTON_LEFT)
        {
             ...
             */
            if (slidebar_ptr->slider_depressed == EG_FALSE && mouse_is_over == OVER_SLIDER)
            {
     ...
     */
    if (event_ptr->type == SDL_MOUSEMOTION && slidebar_ptr->slider_depressed == EG_TRUE)
    {
...
static EG_BOOL Callback_Paint(EG_Widget *widget_ptr, SDL_Rect area)
{
    ...
    if (EG_Shared_GetRenderingDetails(widget_ptr, area, (void *)&slidebar_ptr, &window_ptr, &surface_ptr, &color,
                                      &loc) != EG_TRUE)
        return (EG_TRUE);
...
static void InitializeWidget(EG_Widget *widget_ptr, SDL_Color color, SDL_Rect area)
{
    ...
    (void)EG_Widget_SetCallback_Visible(widget_ptr, EG_Callback_Generic_Visible);

    (void)EG_Widget_SetCallback_Stopped(widget_ptr, EG_Callback_Generic_Stopped);

    (void)EG_Widget_SetCallback_Enabled(widget_ptr, EG_Callback_Generic_Enabled);
    ...
    (void)EG_Widget_SetCallback_Attach(widget_ptr, EG_Callback_Generic_Attach);
...
EG_Widget *EG_SlideBar_Create(const char *name_ptr, SDL_Color color, int orientation, SDL_Rect area)
{
    ...
    SHARED__ALLOC_PAYLOAD_STRUCT(ptr, EG_SlideBar, "Unable to malloc EG_SlideBar struct");
    slidebar_ptr = (EG_SlideBar *)ptr;

    SHARED__CREATE_NEW_EG_WIDGET(widget_ptr, name_ptr, EG_Widget_Type_SlideBar, slidebar_ptr);
...
EG_BOOL EG_SlideBar_SetPosition(EG_Widget *widget_ptr, long position)
{
    ...
    if (position > slidebar_ptr->virtual_length - CalcVisibleLength(widget_ptr))
        position = slidebar_ptr->virtual_length - CalcVisibleLength(widget_ptr);
...
long EG_SlideBar_GetPosition(EG_Widget *widget_ptr)
{
    ...
    if (slidebar_ptr->snap == EG_TRUE)
        position = position / slidebar_ptr->step * slidebar_ptr->step;
