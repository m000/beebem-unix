...
EG_BOOL EG_Malloc_CanFindAllocationInList(void *p)
{
    ...
    for (i = 0; i < EG_DEBUG_MAXMALLOCS; i++)
    {
        ...
        pointer_lowerboundry = EG_CAST_PTR_TO_INT(EG_Malloc_AllocationList[i].ptr);

        pointer_upperboundry = pointer_lowerboundry + (EG_PTR_AS_INT)EG_Malloc_AllocationList[i].size - 1;

        if (pointer >= pointer_lowerboundry && pointer <= pointer_upperboundry)
            return (EG_TRUE);
    }
    return (EG_FALSE);
}
...
void *EG_Malloc(size_t size)
{
    ...
    if ((ptr = (char *)malloc(size)) != NULL)
    {
        ...
        EG_Log(EG_LOG_DEBUG5, dL "malloc(%08ld) = " EG_PTRFMT " [%08lu]", dR, (long)size, EG_CAST_PTR_TO_INT(ptr),
               EG_Malloc_AllocationCount);
    ...
    {
        EG_Log(EG_LOG_DEBUG, dL "Unable to malloc %ld bytes (ALL YOUR BYTES ARE BELONG TO US).", dR, (long)size);
    }
    ...
    ptr_tmp = (int *)ptr;
    *ptr_tmp = -1;
    ptr_tmp = (int *)(ptr + size - (sizeof(int)));
    *ptr_tmp = -2;
...
void EG_Free(void *ptr)
{
    ...
    if ((len = EG_Malloc_GetSize(c_ptr)) > 0)
    {
        ...
        EG_Malloc_AllocationCount--;
        EG_Log(EG_LOG_DEBUG5, dL "free(" EG_PTRFMT ") [%lu]", dR, EG_CAST_PTR_TO_INT(c_ptr), EG_Malloc_AllocationCount);
    ...
    {
        EG_Log(EG_LOG_FATAL, dL "Could not find [" EG_PTRFMT "] in malloc ref, shit..", dR, EG_CAST_PTR_TO_INT(c_ptr));
    }
