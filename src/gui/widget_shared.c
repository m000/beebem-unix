 ...
 * itself.
 */
EG_BOOL EG_Callback_Generic_Visible_NoSupport(EG_Widget *widget_ptr, EG_BOOL is_visible)
{
    /* Loose GNU CC -Wall -W compiler warnings.
     */
...
EG_BOOL EG_Callback_Generic_GotFocus(EG_Widget *widget_ptr)
{
     ...
     */
    if (EG_Widget_IsEnabled(widget_ptr) == EG_FALSE || EG_Widget_IsVisible(widget_ptr) == EG_FALSE)
    {
        EG_Log(EG_LOG_WARNING,
               dL "Tried to set focus to widget '%s'"
 ...
 */
void EG_Callback_Generic_Attach(EG_Widget *widget_ptr, EG_StringHash attach_to_type, void *attach_to_ptr,
                                EG_BOOL attached)
{
 ...
 *
 */
EG_BOOL EG_Shared_GetRenderingDetails(EG_Widget *widget_ptr, SDL_Rect area, void **payload_ptr_ptr // EG_Box *box_ptr;
                                      ,
                                      ...
                                      SDL_Rect *loc_ptr) // SDL_Rect loc;
{
    ...
    {
        EG_Log(EG_LOG_WARNING,
               dL "Tried to render '%s', even though"
                  " EG_Widget_IsVisible == EG_FALSE.",
               dR, EG_Widget_GetName(widget_ptr));
        return (EG_FALSE);
    }
...
//
#define overlap(a, b)                                                                                                  \
    (((((a.y - (b.y + b.h)) ^ ((a.y + a.h) - b.y)) & ((a.x - (b.x + b.w)) ^ ((a.x + a.w) - b.x))) < 0) ? 1 : 0)
    //
    //	if ( overlap(a,b) ){
    ////		printf("OVERLAP\n");
    ...
    //
    //
    //#define overlap(a, b) ( ( ( ((a.y - (b.y+b.h) ) ^ ( (a.y+a.h) - b.y)) & ((a.x - (b.x+b.w) ) ^ ( (a.x+a.w) - b.x))
    //) < 0) ? 1 : 0)
    //
    //	if ( overlap(a,b) )
    //		printf("OVERLAP\n");
                                  ...
                                  EG_BOOL *mouse_over_widget) // EG_BOOL mouse_over_widget;
{
    ...
    {
        EG_Log(EG_LOG_WARNING,
               dL "Tried to process a NULL SDL Event"
                  "'%s'",
               dR, EG_Widget_GetName(widget_ptr));
        return (EG_FALSE);
    }
    ...
    {
        EG_Log(EG_LOG_WARNING,
               dL "Tried to process SDL Event for a"
                  " NULL widget.",
               dR);
        return (EG_FALSE);
    }
    ...
    *window_ptr_ptr = (EG_Window *)EG_Widget_GetWindow(widget_ptr);
    ...
    {
        EG_Log(EG_LOG_WARNING,
               dL "Tried to process an SDL event for"
...
EG_BOOL EG_Shared_UpdateEnabledState(EG_Widget *widget_ptr, EG_BOOL is_enabled)
{
    ...
    {
        EG_Log(EG_LOG_ERROR,
               dL "Changing widget '%s' enabled state"
                  " failed (returned false).",
               dR, EG_Widget_GetName(widget_ptr));
