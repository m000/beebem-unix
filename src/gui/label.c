...
static EG_BOOL Callback_Paint(EG_Widget *widget_ptr, SDL_Rect area)
{
    ...
    if (EG_Shared_GetRenderingDetails(widget_ptr, area, (void *)&label_ptr, &window_ptr, &surface_ptr, &color, &loc) !=
        EG_TRUE)
        return (EG_TRUE);
    ...
    EG_Draw_String(surface_ptr, &color, EG_FALSE, &loc, label_ptr->alignment, label_ptr->caption);
                             ...
                             const char *caption_ptr)
{
    ...
    (void)EG_Widget_SetCallback_Enabled(widget_ptr, EG_Callback_Generic_Enabled_NoSupport);
...
EG_Widget *EG_Label_Create(const char *name_ptr, SDL_Color color, int alignment, const char *caption_ptr,
                           SDL_Rect dimension)
{
    EG_Label *label_ptr;
     ...
     */
    SHARED__ALLOC_PAYLOAD_STRUCT(ptr, EG_Label, "Unable to malloc EG_Label struct");
    label_ptr = (EG_Label *)ptr;

    SHARED__CREATE_NEW_EG_WIDGET(widget_ptr, name_ptr, EG_Widget_Type_Label, label_ptr);
...
EG_BOOL EG_Label_SetCaption(EG_Widget *widget_ptr, const char *caption_ptr)
{
    ...
    if (caption_ptr != NULL)
    {
        if (strlen(caption_ptr) <= MAX_LABEL_CAPTIONSIZE)
            ...
            strncpy(label_ptr->caption, caption_ptr, MAX_LABEL_CAPTIONSIZE);
    }
