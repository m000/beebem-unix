...
static EG_BOOL Callback_Paint(EG_Widget *widget_ptr, SDL_Rect area)
{
    ...
    if (EG_Shared_GetRenderingDetails(widget_ptr, area, (void *)&box_ptr, &window_ptr, &surface_ptr, &color, &loc) !=
        EG_TRUE)
        return (EG_TRUE);
...
static EG_BOOL InitializeWidget(EG_Widget *widget_ptr, SDL_Color color, SDL_Rect dimension, int type)
{
...
EG_Widget *EG_Box_Create(const char *name_ptr, int type, SDL_Color color, SDL_Rect dimension)
{
     ...
     */
    SHARED__ALLOC_PAYLOAD_STRUCT(ptr, EG_Box, "Unable to malloc EG_Box struct");
    box_ptr = (EG_Box *)ptr;

    SHARED__CREATE_NEW_EG_WIDGET(widget_ptr, name_ptr, EG_Widget_Type_Box, box_ptr);
