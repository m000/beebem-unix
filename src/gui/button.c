...
static EG_BOOL Callback_Paint(EG_Widget *widget_ptr, SDL_Rect area)
{
    ...
    if (EG_Shared_GetRenderingDetails(widget_ptr, area, (void *)&button_ptr, &window_ptr, &surface_ptr, &color, &loc) !=
        EG_TRUE)
        return (EG_TRUE);
    ...
    EG_Draw_String(surface_ptr, &color, bold, &loc, button_ptr->alignment, button_ptr->caption);
...
static EG_BOOL Callback_SDL_Event(EG_Widget *widget_ptr, SDL_Event *event_ptr)
{
     ...
     * false.
     */
    if (EG_Shared_GetEventDetails(widget_ptr, event_ptr, (void *)&button_ptr, &window_ptr, &mouse_over_widget) !=
        EG_TRUE)
        return (return_value);
    ...
    if (event_ptr->type == SDL_MOUSEBUTTONDOWN || event_ptr->type == SDL_MOUSEBUTTONUP)
    {
         ...
         */
        if (button_ptr->depressed == EG_FALSE && event_ptr->button.state == SDL_PRESSED &&
         ...
         */
        if (button_ptr->depressed == EG_TRUE && event_ptr->button.state == SDL_RELEASED &&
...
static void InitializeWidget(EG_Widget *widget_ptr, SDL_Color color, SDL_Rect dimension, int alignment,
...
EG_Widget *EG_Button_Create(const char *name_ptr, SDL_Color color, int alignment, const char *caption_ptr,
                            SDL_Rect dimension)
{
    EG_Button *button_ptr;
    ...
    SHARED__ALLOC_PAYLOAD_STRUCT(ptr, EG_Button, "Unable to malloc EG_Button struct");
    button_ptr = (EG_Button *)ptr;

    SHARED__CREATE_NEW_EG_WIDGET(widget_ptr, name_ptr, EG_Widget_Type_Button, button_ptr);
...
EG_BOOL EG_Button_SetCaption(EG_Widget *widget_ptr, const char *caption_ptr)
{
    ...
    if (caption_ptr != NULL)
    {
        if (strlen(caption_ptr) <= MAX_BUTTON_CAPTIONSIZE)
            ...
            strncpy(button_ptr->caption, caption_ptr, MAX_BUTTON_CAPTIONSIZE);
    }
