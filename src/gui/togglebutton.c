...
static EG_BOOL Callback_Paint(EG_Widget *widget_ptr, SDL_Rect area)
{
    ...
    if (EG_Shared_GetRenderingDetails(widget_ptr, area, (void *)&togglebutton_ptr, &window_ptr, &surface_ptr, &color,
                                      &loc) != EG_TRUE)
        return (EG_TRUE);
    ...
    {
        EG_Draw_Toggle(surface_ptr, &loc, &color, EG_Draw_Border_BoxLow);
    }
    else
    {
        EG_Draw_Toggle(surface_ptr, &loc, &color, EG_Draw_Border_BoxHigh);
    }
...
static EG_BOOL Callback_SDL_Event(EG_Widget *widget_ptr, SDL_Event *event_ptr)
{
     ...
     * false.
     */
    if (EG_Shared_GetEventDetails(widget_ptr, event_ptr, (void *)&togglebutton_ptr, &window_ptr, &mouse_over_widget) !=
        EG_TRUE)
        return (return_value);
    ...
    if (event_ptr->type == SDL_MOUSEBUTTONDOWN || event_ptr->type == SDL_MOUSEBUTTONUP)
    {
         ...
         */
        if (togglebutton_ptr->depressed == EG_FALSE && event_ptr->button.state == SDL_PRESSED &&
         ...
         */
        if (togglebutton_ptr->depressed == EG_TRUE && event_ptr->button.state == SDL_RELEASED &&
            event_ptr->button.button == SDL_BUTTON_LEFT)
        {
             ...
             */
            if (mouse_over_widget == EG_TRUE && EG_Widget_GetVisibleToggle(widget_ptr) == EG_TRUE &&
...
EG_Widget *EG_ToggleButton_Create(const char *name_ptr, SDL_Color color, const char *caption_ptr, SDL_Rect dimension)
{
    EG_ToggleButton *togglebutton_ptr;
    EG_Widget *widget_ptr;
    void *ptr;

    SHARED__ALLOC_PAYLOAD_STRUCT(ptr, EG_ToggleButton, "Unable to malloc EG_ToggleButton struct");
    togglebutton_ptr = (EG_ToggleButton *)ptr;

    SHARED__CREATE_NEW_EG_WIDGET(widget_ptr, name_ptr, EG_Widget_Type_ToggleButton, togglebutton_ptr);
...
EG_BOOL EG_ToggleButton_SetCaption(EG_Widget *widget_ptr, const char *caption_ptr)
{
    ...
    if (caption_ptr != NULL)
    {
        if (strlen(caption_ptr) <= MAX_TOGGLEBUTTON_CAPTIONSIZE)
            ...
            strncpy(togglebutton_ptr->caption, caption_ptr, MAX_TOGGLEBUTTON_CAPTIONSIZE);
    }
