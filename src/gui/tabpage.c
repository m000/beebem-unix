...
static void Callback_Destroy(EG_Widget *widget_ptr)
{
    ...
    {
        EG_Log(EG_LOG_ERROR,
               dL "Tried to Destroy EG_TabPage with"
                  " NULL widget pointer.",
               dR);
        return;
    }
...
static EG_BOOL Callback_Paint(EG_Widget *widget_ptr, SDL_Rect area)
{
     ...
     */
    if (EG_Shared_GetRenderingDetails(widget_ptr, area, (void *)&tabpage_ptr, &window_ptr, &surface_ptr, &color,
                                      &loc) != EG_TRUE)
        return (EG_TRUE);
    ...
    loc.x += 1;
    loc.w -= 2;
    loc.y += EG_TabGroup_PageButtonHeight - 1;
    if (EG_TabPage_IsShown(widget_ptr) == EG_TRUE)
    {
        loc.h = 2;
    }
...
static EG_BOOL Callback_SDL_Event(EG_Widget *widget_ptr, SDL_Event *event_ptr)
{
     ...
     * false.
     */
    if (EG_Shared_GetEventDetails(widget_ptr, event_ptr, (void *)&tabpage_ptr, &window_ptr, &mouse_over_widget) !=
        EG_TRUE)
        return (return_value);
    ...
    if (event_ptr->type == SDL_MOUSEBUTTONDOWN || event_ptr->type == SDL_MOUSEBUTTONUP)
    {
         ...
         */
        if (tabpage_ptr->depressed == EG_FALSE && event_ptr->button.state == SDL_PRESSED &&
            event_ptr->button.button == SDL_BUTTON_LEFT && mouse_over_widget == EG_TRUE)
        {
         ...
         */
        if (tabpage_ptr->depressed == EG_TRUE && event_ptr->button.state == SDL_RELEASED &&
...
static EG_BOOL Callback_Visible(EG_Widget *widget_ptr, EG_BOOL is_visible)
{
    ...
    for (i = 0; i < tabpage_ptr->count; i++)
    {
        (void)EG_Widget_CallVisible(tabpage_ptr->child[i].widget_ptr, is_visible);
    }
...
static EG_BOOL Callback_Enabled(EG_Widget *widget_ptr, EG_BOOL is_enabled)
{
    ...
    for (i = 0; i < tabpage_ptr->count; i++)
    {
        (void)EG_Widget_CallEnabled(tabpage_ptr->child[i].widget_ptr, is_enabled);
    }
...
// [TODO] This is fixed now, (forgot to do nothing when detaching {we don't need to do anything for that in this version
// of EG as the window micro-manages everything, in the final version where parent widgets manage child widgets [thus
// keeping their implementation secret from the parent], so they will need to detatch their children from window
...
static void InitializePayload(EG_TabPage *tabpage_ptr, const char *caption_ptr)
{
    ...
    if (caption_ptr != NULL)
    {
        if (strlen(caption_ptr) <= MAX_TABPAGE_CAPTIONSIZE)
            ...
            strncpy(tabpage_ptr->caption, caption_ptr, MAX_TABPAGE_CAPTIONSIZE);
    }
...
EG_Widget *EG_TabPage_Create(const char *name_ptr, const char *caption_ptr)
{
    ...
    SHARED__ALLOC_PAYLOAD_STRUCT(tabpage_ptr, EG_TabPage, "Unable to malloc EG_TabPage struct");

    SHARED__CREATE_NEW_EG_WIDGET(widget_ptr, name_ptr, EG_Widget_Type_TabPage, tabpage_ptr);
...
EG_BOOL EG_TabPage_AddWidget(EG_Widget *page_widget_ptr, EG_Widget *child_widget_ptr)
{
    ...
    {
        EG_Log(EG_LOG_ERROR,
               dL "EG_Widget is NULL, cannot add it to"
                  " EG_TabPage.",
               dR);
        return (EG_FALSE);
    }
    ...
    {
        EG_Log(EG_LOG_ERROR,
               dL "Can't add EG_Widget to EG_TabPage: "
                  "Would exceed max widget count.",
               dR);
        return (EG_FALSE);
    }
    ...
    if (EG_Widget_GetStoppedToggle(page_widget_ptr) == EG_TRUE)
    {
        ...
        if (EG_Window_StopWidget(child_widget_ptr) != EG_TRUE)
            EG_Log(EG_LOG_ERROR, dL "Could not stop child widget.", dR);
    }
    else
    {
        ...
        if (EG_TabPage_IsShown(page_widget_ptr) == EG_TRUE)
        {
            ...
            if (EG_Window_StartWidget(child_widget_ptr) != EG_TRUE)
                EG_Log(EG_LOG_ERROR, dL "Could not start child widget.", dR);
        }
        else
        {
...
EG_BOOL EG_TabPage_Show(EG_Widget *widget_ptr)
{
     ...
     */
    for (i = 0; i < tabpage_ptr->count; i++)
        if (EG_Window_StartWidget(tabpage_ptr->child[i].widget_ptr) != EG_TRUE)
...
EG_BOOL EG_TabPage_Hide(EG_Widget *widget_ptr)
{
    ...
    window_ptr = EG_Widget_GetWindow(widget_ptr);
    if (window_ptr != NULL)
        focused_widget_ptr = EG_Window_Child_GetFocusedWidget(window_ptr);
    else
    ...
    for (i = 0; i < tabpage_ptr->count; i++)
    {
        ...
        if (EG_Window_StopWidget(tabpage_ptr->child[i].widget_ptr) != EG_TRUE)
...
EG_BOOL EG_TabPage_RePaintChildren(EG_Widget *widget_ptr)
{
     ...
     */
    for (i = 0; i < tabpage_ptr->count; i++)
        if (EG_Widget_RepaintLot(tabpage_ptr->child[i].widget_ptr) != EG_TRUE)
            EG_Log(EG_LOG_ERROR,
                   dL "Failed to repaint tab page"
                      " child widget.",
                   dR);
...
EG_BOOL EG_TabPage_IsShown(EG_Widget *widget_ptr)
{
    ...
    selected_widget_ptr = EG_TabGroup_GetSelectedPageWidget(tabpage_ptr->tabgroup_widget_ptr);

    if (EG_Widget_GetID(selected_widget_ptr) == EG_Widget_GetID(widget_ptr))
        return (EG_TRUE);
    else
        return (EG_FALSE);
}
