...
// [TODO] Probably a bit late now, but add the type as an argument so it can test that it is the correct type of widget
// too.
#ifdef EG_DEBUG
#define CHECK_EG_WIDGET_IS_VALID(o, r)                                                                                 \
    if (o == NULL)                                                                                                     \
    {                                                                                                                  \
        EG_Log(EG_LOG_WARNING, dL "EG_Widget pointer is NULL.", dR);                                                   \
        return (r);                                                                                                    \
    }                                                                                                                  \
    EG_ASSERT_MALLOC(o);
...
extern "C"
{
    ...
    void EG_Widget_CallAttach(EG_Widget *widget_ptr, EG_StringHash attach_to_type, void *attach_to_ptr,
                              EG_BOOL attached);
