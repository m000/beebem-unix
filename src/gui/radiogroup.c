...
static void Callback_Destroy(EG_Widget *widget_ptr)
{
    ...
    {
        EG_Log(EG_LOG_ERROR,
               dL "Tried to Destroy Radio Group with"
                  " widget_ptr=NULL",
               dR);
        return;
    }
    ...
    {
        EG_Log(EG_LOG_ERROR,
               dL "EG_RadioGroup Destroy callback called"
                  " for none EG_RadioGroup widget.",
               dR);
    }
    else
    {
        ...
        if ((radiogroup_ptr = (EG_RadioGroup *)EG_Widget_GetPayload(widget_ptr)) != NULL)
        {
            for (i = 0; i < radiogroup_ptr->count; i++)
            {
                ...
                default:
                    EG_Log(EG_LOG_ERROR, dL "Widget %s is of incorrect type.", dR, EG_Widget_GetName(tmp_widget_ptr));
                    break;
                }
...
static EG_BOOL Callback_Paint(EG_Widget *widget_ptr, SDL_Rect area)
{
     ...
     */
    for (i = 0; i < radiogroup_ptr->count; i++)
        (void)EG_Widget_RepaintLot(radiogroup_ptr->button_widget_ptr[i]);
...
static EG_BOOL Callback_Stopped(EG_Widget *widget_ptr, EG_BOOL is_stopped)
{
    ...
    for (i = 0; i < radiogroup_ptr->count; i++)
    {
        EG_Widget_SetStoppedToggle(radiogroup_ptr->button_widget_ptr[i], is_stopped);
    }
...
static void Callback_Attach(EG_Widget *widget_ptr, EG_StringHash attach_to_type, void *attach_to_ptr, EG_BOOL attached)
{
    ...
    for (i = 0; i < radiogroup_ptr->count; i++)
    {
        EG_Window_AddWidget(window_ptr, radiogroup_ptr->button_widget_ptr[i]);
    }
}
...
EG_Widget *EG_RadioGroup_Create(const char *name_ptr)
{
    ...
    SHARED__ALLOC_PAYLOAD_STRUCT(ptr, EG_RadioGroup, "Unable to malloc EG_RadioGroup struct");
    radiogroup_ptr = (EG_RadioGroup *)ptr;

    SHARED__CREATE_NEW_EG_WIDGET(widget_ptr, name_ptr, EG_Widget_Type_RadioGroup, radiogroup_ptr);
...
EG_BOOL EG_RadioGroup_AddButton(EG_Widget *group_widget_ptr, EG_Widget *button_widget_ptr)
{
    ...
    {
        EG_Log(EG_LOG_ERROR,
               dL "Can't add button to"
                  " EG_RadioGroup: Would exceed max button count.",
               dR);
        return (EG_FALSE);
    }
    ...
    default:
        EG_Log(EG_LOG_ERROR,
               dL "Can't attach child to EG_RadioGroup: "
                  "It's not of correct type.",
               dR);
        return EG_FALSE;
        break;
    }
...
void EG_RadioGroup_RemoveButton(EG_Widget *group_widget_ptr, EG_Widget *button_widget_ptr)
{
     ...
     */
    for (i = 0; i < radiogroup_ptr->count; i++)
        if (EG_Widget_GetID(button_widget_ptr) == EG_Widget_GetID(radiogroup_ptr->button_widget_ptr[i]))
            break;
    ...
    if (i >= radiogroup_ptr->count)
    {
        EG_Log(EG_LOG_ERROR,
               dL "Tried to delete widget %s from"
                  " EG_RadioGroup, when not memeber of group.",
               dR);
        return;
    }
...
EG_BOOL EG_RadioGroup_Select(EG_Widget *button_widget_ptr)
{
    ...
    case EG_Widget_Type_RadioButton:
        ...
        {
            EG_Log(EG_LOG_ERROR,
                   dL "EG_RadioButton is not associated with"
                      " an EG_RadioGroup.",
                   dR);
            return EG_FALSE;
        }
    ...
    case EG_Widget_Type_ToggleButton:
        ...
        {
            EG_Log(EG_LOG_ERROR,
                   dL "EG_ToggleButton is not associated with"
                      " an EG_RadioGroup.",
                   dR);
            return EG_FALSE;
        }
    ...
    for (i = 0; i < radiogroup_ptr->count; i++)
    {
        ...
        {
            if (EG_Widget_GetID(next_button_widget_ptr) == EG_Widget_GetID(button_widget_ptr))
                break;
        }
    }
    if (i >= radiogroup_ptr->count)
    {
        EG_Log(EG_LOG_ERROR,
               dL "Button not on EG_RadioGroup"
                  " list.",
               dR);
        return (EG_FALSE);
    }
        ...
        if (EG_Widget_GetID(next_button_widget_ptr) != EG_Widget_GetID(button_widget_ptr))
        {
            ...
            case EG_Widget_Type_ToggleButton:
                if (EG_ToggleButton_SetUnselected(next_button_widget_ptr) != EG_TRUE)
                    EG_Log(EG_LOG_ERROR, dL "EG_ToggleButton could not be unselected.", dR);
                break;
            }
            ...
            if (window_ptr != NULL)
            {
                if (EG_Window_SetFocusToThisWidget(button_widget_ptr) != EG_TRUE)
                {
                    ...
                    EG_Log(EG_LOG_WARNING,
                           dL "Could not move focus to pressed"
                              " button.",
                           dR);
                }
            }
