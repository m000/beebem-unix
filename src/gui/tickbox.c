...
static EG_BOOL Callback_Paint(EG_Widget *widget_ptr, SDL_Rect area)
{
    ...
    if (EG_Shared_GetRenderingDetails(widget_ptr, area, (void *)&tickbox_ptr, &window_ptr, &surface_ptr, &color,
                                      &loc) != EG_TRUE)
        return (EG_TRUE);
...
static EG_BOOL Callback_SDL_Event(EG_Widget *widget_ptr, SDL_Event *event_ptr)
{
     ...
     * false.
     */
    if (EG_Shared_GetEventDetails(widget_ptr, event_ptr, (void *)&tickbox_ptr, &window_ptr, &mouse_over_widget) !=
        EG_TRUE)
        return (return_value);
    ...
    if (event_ptr->type == SDL_MOUSEBUTTONDOWN || event_ptr->type == SDL_MOUSEBUTTONUP)
    {
         ...
         */
        if (tickbox_ptr->depressed == EG_FALSE && event_ptr->button.state == SDL_PRESSED &&
         ...
         */
        if (tickbox_ptr->depressed == EG_TRUE && event_ptr->button.state == SDL_RELEASED &&
...
EG_Widget *EG_TickBox_Create(const char *name_ptr, SDL_Color color, const char *caption_ptr, SDL_Rect dimension)
{
    EG_TickBox *tickbox_ptr;
    EG_Widget *widget_ptr;
    void *ptr;

    SHARED__ALLOC_PAYLOAD_STRUCT(ptr, EG_TickBox, "Unable to malloc EG_TickBox struct");
    tickbox_ptr = (EG_TickBox *)ptr;

    SHARED__CREATE_NEW_EG_WIDGET(widget_ptr, name_ptr, EG_Widget_Type_TickBox, tickbox_ptr);
...
EG_BOOL EG_TickBox_SetCaption(EG_Widget *widget_ptr, const char *caption_ptr)
{
    ...
    if (caption_ptr != NULL)
    {
        if (strlen(caption_ptr) <= MAX_TICKBOX_CAPTIONSIZE - 2)
            ...
            strncpy(tickbox_ptr->caption + 2, caption_ptr, MAX_TICKBOX_CAPTIONSIZE - 2);
    }
