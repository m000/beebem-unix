 ...
 */
static void EG_Draw_UpdateSurfaceAbsolute(SDL_Surface *surface, Sint32 x, Sint32 y, Sint32 w, Sint32 h)
{
...
{ // int i;
    ...
    // Set color key
    SDL_SetColorKey(label_low, SDL_SRCCOLORKEY, SDL_MapRGB(label_low->format, 255, 255, 255));
    SDL_SetColorKey(label_high, SDL_SRCCOLORKEY, SDL_MapRGB(label_high->format, 255, 255, 255));
...
void EG_Draw_String(SDL_Surface *surface, SDL_Color *color, EG_BOOL bold, SDL_Rect *area_ptr, int justify, char *string)
{
...
void EG_Draw_TabBorder(SDL_Surface *surface, SDL_Rect *area, SDL_Color *color, int type)
{
    ...
    case EG_Draw_Border_Normal:
        bright_col = SDL_MapRGB(surface->format, (Uint8)color->r, (Uint8)color->g, (Uint8)color->b);

        dull_col = bright_col;
        break;

    case EG_Draw_Border_BoxHigh:
        bright_col = SDL_MapRGB(surface->format, (int)(1.3333 * color->r > 255.0 ? 255.0 : 1.3333 * color->r),
                                (int)(1.3333 * color->g > 255.0 ? 255.0 : 1.3333 * color->g),
                                (int)(1.3333 * color->b > 255.0 ? 255.0 : 1.3333 * color->b));

        dull_col =
            SDL_MapRGB(surface->format, (int)(color->r * 0.6666), (int)(color->g * 0.6666), (int)(color->b * 0.6666));
        break;
    ...
    case EG_Draw_Border_Focused:
        ...
        dull_col = SDL_MapRGB(surface->format, (int)(color->r * 0.4), (int)(color->g * 0.4), (int)(color->b * 0.4));
    ...
    // Top line:
    line.x = drawing_area.x + 1;
    line.y = drawing_area.y;
    ...
    // Bottom line:
    line.x = drawing_area.x + 1;
    line.y = drawing_area.y + drawing_area.h - 1;
    line.w = drawing_area.w - 2;
    line.h = 1;
    //       SDL_FillRect(surface, &line, dull_col);

    // Left line:
    line.x = drawing_area.x;
    line.y = drawing_area.y + 1;
    ...
    // Right line:
    line.x = drawing_area.x + drawing_area.w - 1;
    line.y = drawing_area.y + 1;
    line.w = 1;
    line.h = drawing_area.h - 2;
    ...
    EG_Draw_UpdateSurfaceAbsolute(surface, drawing_area.x, drawing_area.y, drawing_area.w, drawing_area.h + 2);
}
...
void EG_Draw_Border(SDL_Surface *surface, SDL_Rect *area, SDL_Color *color, int type)
{
    ...
    case EG_Draw_Border_BoxLow:
        dull_col = SDL_MapRGB(surface->format, (int)(1.3333 * color->r > 255.0 ? 255.0 : 1.3333 * color->r),
                              (int)(1.3333 * color->g > 255.0 ? 255.0 : 1.3333 * color->g),
                              (int)(1.3333 * color->b > 255.0 ? 255.0 : 1.3333 * color->b));
    ...
    case EG_Draw_Border_Focused:
        dull_col = SDL_MapRGB(surface->format, (int)(color->r * 0.7), (int)(color->g * 0.7), (int)(color->b * 0.7));
...
void EG_Draw_Toggle(SDL_Surface *surface, SDL_Rect *area, SDL_Color *color, int type)
{
    ...
    line.x = drawing_area.x;
    line.y = drawing_area.y;
    line.w = drawing_area.w;
    line.h = drawing_area.h - corner;
    SDL_FillRect(surface, &line, fill_col);

    line.x = drawing_area.x;
    line.y = drawing_area.y + drawing_area.h - corner;
    line.w = drawing_area.w;
    line.h = 1;
    ...
    // Bottom line:
    line.x = drawing_area.x + 1 + corner;
    line.y = drawing_area.y + drawing_area.h - 1;
    ...
    line.x = drawing_area.x;
    line.y = drawing_area.y + 1;
    line.h = drawing_area.h - 2 - corner;
    line.w = 1;
    SDL_FillRect(surface, &line, bright_col);
