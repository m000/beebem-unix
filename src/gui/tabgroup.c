...
static void Callback_Destroy(EG_Widget *widget_ptr)
{
    ...
    {
        EG_Log(EG_LOG_ERROR,
               dL "Tried to Destroy EG_TabGroup with"
                  " NULL widget pointer.",
               dR);
        return;
    }
     ...
     */
    if (tabgroup_ptr != NULL)
        EG_Free(tabgroup_ptr);
...
static EG_BOOL Callback_Paint(EG_Widget *widget_ptr, SDL_Rect area)
{
     ...
     */
    if (EG_Shared_GetRenderingDetails(widget_ptr, area, (void *)&tabgroup_ptr, &window_ptr, &surface_ptr, &color,
                                      &loc) != EG_TRUE)
        return (EG_TRUE);
    ...
    printf("SOMEONE CALLED PAINT FOR '%s' [repaint area (window)="
           ...
           EG_Widget_GetName(widget_ptr), area.x, area.y, area.w, area.h, loc.x, loc.y, loc.w, loc.h);
#endif
...
static EG_BOOL Callback_Visible(EG_Widget *widget_ptr, EG_BOOL is_visible)
{
    ...
    {
        EG_Log(EG_LOG_ERROR,
               dL "Changing widget '%s' visible state"
    ...
    for (i = 0; i < tabgroup_ptr->count; i++)
    {
        (void)EG_Widget_CallVisible(tabgroup_ptr->page_widget_ptr[i], is_visible);
    }
...
static EG_BOOL Callback_Enabled(EG_Widget *widget_ptr, EG_BOOL is_enabled)
{
    ...
    for (i = 0; i < tabgroup_ptr->count; i++)
    {
        (void)EG_Widget_CallEnabled(tabgroup_ptr->page_widget_ptr[i], is_enabled);
    }
...
static void Callback_Attach(EG_Widget *widget_ptr, EG_StringHash attach_to_type, void *attach_to_ptr, EG_BOOL attached)
{
    ...
    if (tabgroup_ptr->count > 0)
    {
         ...
         */
        if (EG_TabGroup_SelectPageByIndex(widget_ptr, 1) == EG_FALSE)
            EG_Log(EG_LOG_ERROR,
                   dL "Failed to make EG_TabPage"
                      " active.",
                   dR);
    }
}
...
static void InitializeWidget(EG_Widget *widget_ptr, SDL_Color color, SDL_Rect dimension)
{
    CHECK_EG_WIDGET_IS_VALID_VOID(widget_ptr);
    ...
    (void)EG_Widget_SetCallback_SDL_Event(widget_ptr, EG_Callback_Generic_SDL_Event);
    ...
    (void)EG_Widget_SetCallback_GotFocus(widget_ptr, EG_Callback_Generic_GotFocus_NoSupport);

    (void)EG_Widget_SetCallback_LostFocus(widget_ptr, EG_Callback_Generic_LostFocus_NoSupport);
...
EG_Widget *EG_TabGroup_Create(const char *name_ptr, SDL_Color color, SDL_Rect dimension)
{
    ...
    SHARED__ALLOC_PAYLOAD_STRUCT(tabgroup_ptr, EG_TabGroup, "Unable to malloc EG_TabGroup struct");

    SHARED__CREATE_NEW_EG_WIDGET(widget_ptr, name_ptr, EG_Widget_Type_TabGroup, tabgroup_ptr);
...
EG_BOOL EG_TabGroup_AddPage(EG_Widget *group_widget_ptr, EG_Widget *page_widget_ptr)
{
    ...
    {
        EG_Log(EG_LOG_ERROR,
               dL "Can't attach child to EG_TabGroup: "
                  "It's not an EG_TabPage.",
               dR);
        return (EG_FALSE);
    }
    ...
    {
        EG_Log(EG_LOG_ERROR,
               dL "Can't add EG_TabPage to EG_TabGroup: "
                  "Would exceed max page count.",
               dR);
        return (EG_FALSE);
    }
...
#define SPACER 2
    ...
    if (EG_TabPage_SetTabGroup(page_widget_ptr, group_widget_ptr) != EG_TRUE)
        EG_Log(EG_LOG_ERROR,
               dL "Could not associcate EG_TabPage with"
                  " EG_TabGroup.\n",
               dR);

    /* Set the tab page background color.
     */
    if (EG_Widget_SetBackgroundColor(page_widget_ptr, EG_Widget_GetBackgroundColor(group_widget_ptr)) != EG_TRUE)
        EG_Log(EG_LOG_ERROR,
               dL "Could not set EG_TabPage background"
                  " color.\n",
               dR);
...
void EG_TabGroup_RemovePage(EG_Widget *group_widget_ptr, EG_Widget *page_widget_ptr)
{
     ...
     */
    for (i = 0; i < tabgroup_ptr->count; i++)
        if (EG_Widget_GetID(page_widget_ptr) == EG_Widget_GetID(tabgroup_ptr->page_widget_ptr[i]))
            break;
    ...
    {
        EG_Log(EG_LOG_ERROR,
               dL "Tried to delete widget %s from"
                  " EG_TabGroup, when not memeber of group.",
               dR);
        return;
    }
    ...
    // [TODO] Set tabgroups page to page after removed page, if we deleted the last page, set tabgroups page to the
    // previous page.  If this was the only page, render only the tabgroup (will paint background). [TODO] If a widget
    // on the page had focus before we removed it, set focus to the next page.  If we removed the last page, set focus
    // to the previous page, if we deleted the only page, ask window to movefocus.  (will set to first widget).
...
EG_BOOL EG_TabGroup_SelectPage(EG_Widget *page_widget_ptr)
{
    ...
    for (i = 0; i < tabgroup_ptr->count; i++)
    {
        ...
        {
            if (EG_Widget_GetID(next_page_widget_ptr) == EG_Widget_GetID(page_widget_ptr))
            {
                return (EG_TabGroup_SelectPageByIndex(EG_TabPage_GetTabGroup(page_widget_ptr), i + 1));
            }
        }
    }
    return (EG_FALSE);
...
EG_BOOL EG_TabGroup_SelectPageByIndex(EG_Widget *widget_ptr, int page)
{
    ...
    /* [TODO] Clear the windows focus otherwise repainting removes tail of button thats just lost focus..  This should
     * be fixed..
     */
    ...
    if (tabgroup_ptr->selected >= 0)
    {
         ...
         */
        old_selected_tab_page_widget = tabgroup_ptr->page_widget_ptr[tabgroup_ptr->selected];

        if (EG_TabPage_Hide(tabgroup_ptr->page_widget_ptr[tabgroup_ptr->selected]) != EG_TRUE)
        {
            EG_Log(EG_LOG_ERROR, dL "Tab page failed to hide.", dR);
            return (EG_FALSE);
        }
    }
    ...
    if (EG_TabPage_Show(tabgroup_ptr->page_widget_ptr[page - 1]) != EG_TRUE)
        EG_Log(EG_LOG_ERROR,
               dL "Tab page failed to show, but assuming"
                  " it did.",
               dR);
    else if (EG_TabPage_RePaintChildren(tabgroup_ptr->page_widget_ptr[page - 1]) != EG_TRUE)
        EG_Log(EG_LOG_ERROR,
               dL "Tab page failed to repaint, but"
                  " assuming it did.",
               dR);
     ...
     * been de-selected.
     */
    EG_Widget_RepaintLot(tabgroup_ptr->page_widget_ptr[tabgroup_ptr->selected]);
...
EG_Widget *EG_TabGroup_GetSelectedPageWidget(EG_Widget *widget_ptr)
{
    ...
    if (tabgroup_ptr->selected < 0 || tabgroup_ptr->selected >= MAX_TABGROUP_PAGES)
        return (NULL);
    else
