...
static EG_BOOL Callback_Paint(EG_Widget *widget_ptr, SDL_Rect area)
{
    ...
    if (EG_Shared_GetRenderingDetails(widget_ptr, area, (void *)&progressbar_ptr, &window_ptr, &surface_ptr, &color,
                                      &loc) != EG_TRUE)
        return (EG_TRUE);
...
EG_Widget *EG_ProgressBar_Create(const char *name_ptr, SDL_Color color, SDL_Color bar_color, SDL_Rect dimension)
{
     ...
     */
    SHARED__ALLOC_PAYLOAD_STRUCT(ptr, EG_ProgressBar, "Unable to malloc EG_ProgressBar struct");
    progressbar_ptr = (EG_ProgressBar *)ptr;

    SHARED__CREATE_NEW_EG_WIDGET(widget_ptr, name_ptr, EG_Widget_Type_ProgressBar, progressbar_ptr);
