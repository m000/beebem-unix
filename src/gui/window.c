...
static EG_BOOL InitializeWindowStruct(EG_Window *window_ptr, const char *name_ptr)
{
    ...
    if (++unique_ID == 0)
    {
        EG_Log(EG_LOG_INFO, dL "EG_Window struct ID counter wrapped.", dR);
        unique_ID++;
    }
     ...
     */
    if (EG_Window_SetName(window_ptr, name_ptr) == EG_FALSE)
        return (EG_FALSE);
...
EG_Window *EG_Window_Alloc(const char *name_ptr)
{
    ...
    {
        EG_Log(EG_LOG_WARNING, dL "Unable to malloc EG_Window struct", dR);
        return (NULL);
    }
...
void EG_Window_DestroyAllChildWidgets(EG_Window *window_ptr)
{
    ...
    while (EG_Window_Child_GetCount(window_ptr) != 0)
    {
        ...
        if (tmp_widget_ptr != NULL)
        {
            ...
            EG_Widget_CallAttach(tmp_widget_ptr, EG_Window_Type_Window, (void *)window_ptr, EG_FALSE);
...
void EG_Window_Free(EG_Window *window_ptr)
{
    ...
    if (window_ptr->count > 0)
        EG_Log(EG_LOG_WARNING, dL "Freeing window when children exist", dR);
...
EG_BOOL EG_Window_Child_SetFocusedIndexByWidget(EG_Window *window_ptr, EG_Widget *widget_ptr)
{
    ...
    if (EG_Window_GetID(window_ptr) != EG_Window_GetID(widget_window_ptr))
    {
        EG_Log(EG_LOG_ERROR,
               dL "Referenced widget is not on this"
                  " window!",
               dR);
        return (-1);
    }
...
EG_Widget *EG_Window_Child_GetFocusedWidget(EG_Window *window_ptr)
{
    ...
    return (EG_Window_Child_Get(window_ptr, window_ptr->widget_with_focus));
}
...
EG_Widget *EG_Window_Child_Get(EG_Window *window_ptr, long from_index)
{
    ...
    {
        EG_Log(EG_LOG_ERROR,
               dL "Tried to access array index out of"
                  " range.",
               dR);
        return (NULL);
    }
...
EG_BOOL EG_Window_Child_Remove(EG_Window *window_ptr, long from_index)
{
    ...
    {
        EG_Log(EG_LOG_ERROR,
               dL "Tried to delete array index out of"
                  " range.",
               dR);
        return (EG_FALSE);
    }
...
EG_BOOL EG_Window_Child_Add(EG_Window *window_ptr, EG_Widget *widget_ptr)
{
    ...
    {
        EG_Log(EG_LOG_ERROR,
               dL "Cannot add more child widgets to window"
                  " '%s', max count met.",
...
EG_BOOL EG_Window_AddWidget(EG_Window *window_ptr, void *widget_ptr)
{
    ...
    if (EG_Window_Child_Add(window_ptr, (EG_Widget *)widget_ptr) == EG_TRUE)
    {
         ...
         */
        EG_Widget_CallAttach((EG_Widget *)widget_ptr, EG_Window_Type_Window, (void *)window_ptr, EG_TRUE);
...
EG_BOOL EG_Window_RemoveWidget(EG_Window *window_ptr, void *widget_ptr)
{
    ...
    {
        EG_Log(EG_LOG_ERROR,
               dL "Cannot remove widget from window, not"
                  " on window",
               dR);
        return (EG_FALSE);
    }
     ...
     */
    EG_Widget_CallAttach((EG_Widget *)widget_ptr, EG_Window_Type_Window, (void *)window_ptr, EG_FALSE);
...
static EG_BOOL MoveFocus(EG_Window *window_ptr, int direction)
{
    ...
    if ((widget_ptr = EG_Window_Child_GetFocusedWidget(window_ptr)) != NULL)
    {
        ...
        if (current_index >= EG_Window_Child_GetCount(window_ptr))
            current_index = 0;
        if (current_index < 0)
            current_index = EG_Window_Child_GetCount(window_ptr) - 1;
    ...
    do
    {
        ...
        {
            if (EG_Window_Child_SetFocusedIndex(window_ptr, current_index) != EG_TRUE)
            {
                EG_Log(EG_LOG_ERROR,
                       dL "Window failed to switch"
...
EG_Window *EG_Window_Create(char *name_ptr, SDL_Surface *surface_ptr, SDL_Color background_color, SDL_Rect dimension)
{
     ...
     */
    if (dimension.x + dimension.y + dimension.w + dimension.h == 0 && surface_ptr != NULL)
    {
        dimension.x = 0;
        dimension.y = 0;
...
EG_BOOL EG_Window_Repaint(EG_Window *window_ptr, SDL_Rect area)
{
    ...
    // printf("area = %d %d %d %d, size = %d %d %d %d\n", area.x, area.y, area.w, area.h, size.x, size.y, size.w,
    // size.h);
     ...
     */
    if (area.x == 0 && area.y == 0 && area.w == size.w && area.h == size.h)
    {
        /* Convert coordinates so they are relative to the SDL_Surface.
         */
        //
        ...
        EG_Draw_Border(surface_ptr, &size, &color, EG_Draw_Border_BoxHigh);
    ...
    for (i = 0; i < EG_Window_Child_GetCount(window_ptr); i++)
    {
        ...
        if (tmp_widget_ptr != NULL)
        {
            ...
            {
                if (EG_Widget_CallPaint(tmp_widget_ptr, area) == EG_FALSE)
                {
                    EG_Log(EG_LOG_ERROR,
                           dL "Repainting"
...
EG_BOOL EG_Window_ProcessEvent(EG_Window *window_ptr, SDL_Event *event_ptr, int x_displacement, int y_displacement)
{
    ...
    // [TODO] Once all widgets converted to widget_shared.c code, make mouse coordinates relative to window coordinates
    // so widget_shared.c code does not have to add the window x,y ofset within the surface. (See
    // EG_Shared_GetEventDetails)
    ...
    if (Process_SDL_Event(window_ptr, event_ptr) != EG_TRUE)
    {
        ...
        for (i = 0; i < EG_Window_Child_GetCount(window_ptr); i++)
        {
            ...
            {
                EG_Log(EG_LOG_ERROR,
                       dL "widget index %d failed (is NULL)"
                          " while sending SDL Event",
                       dR, (int)i);
            }
        }
    }
...
EG_BOOL EG_Window_Hide(EG_Window *window_ptr)
{
    ...
    {
        if (SDL_BlitSurface(window_ptr->window_background_surface_ptr, NULL, window_ptr->surface_ptr, NULL) != 0)
            EG_Log(EG_LOG_ERROR,
                   dL "Window failed to re-store"
                      " background",
                   dR);
        else
            SDL_UpdateRect(window_ptr->surface_ptr, 0, 0, window_ptr->surface_ptr->w, window_ptr->surface_ptr->h);
    ...
    else
    {
        ...
        SDL_FillRect(window_ptr->surface_ptr, &r, SDL_MapRGB(window_ptr->surface_ptr->format, 0, 0, 0));
...
EG_BOOL EG_Window_Show(EG_Window *window_ptr)
{
    ...
    window_ptr->window_background_surface_ptr =
        SDL_ConvertSurface(window_ptr->surface_ptr, window_ptr->surface_ptr->format, SDL_SWSURFACE);
...
EG_BOOL EG_Window_SetFocusToThisWidget(EG_Widget *widget_ptr)
{
    ...
    if (EG_Widget_CallGotFocus(widget_ptr) == EG_TRUE)
    {
         ...
         */
        if (EG_Window_Child_SetFocusedIndexByWidget(window_ptr, widget_ptr) != EG_TRUE)
            EG_Log(EG_LOG_ERROR,
                   dL "Window failed to switch focus"
                      " index to new child widget. This is a bug :-(",
                   dR);
    ...
    else
    {
        ...
        if (focused_widget_ptr != NULL)
        {
            if (EG_Window_Child_SetFocusedIndexByWidget(window_ptr, focused_widget_ptr) != EG_TRUE)
                EG_Log(EG_LOG_ERROR,
                       dL "Window failed to switch focus"
                          " index to new child widget. This is a bug :-(",
                       dR);
