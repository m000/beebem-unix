...
static const char *FormatText(const char *source_ptr, char *next_row_ptr)
{
    ...
    while (strlen(next_row_ptr) > 0 && next_row_ptr[strlen(next_row_ptr) - 1] <= 32)
...
int EG_MessageBox(SDL_Surface *surface_ptr, int type, const char *title_ptr, const char *text_ptr,
                  const char *button1_ptr, const char *button2_ptr, const char *button3_ptr, const char *button4_ptr,
                  int has_focus)
{
    ...
    win = CalcRectCentered_MB(surface_ptr, 40 + 40 * 10, 16 * 5 + ((row_count < 24 ? row_count : 24) * 16));
    ...
    widget_ptr = EG_Label_Create("lab_msgbox", col, EG_LABEL_ALIGN_CENTER, title_ptr, loc);

    (void)EG_Window_AddWidget(window_ptr, widget_ptr);

    loc = CalcRect(5 + 40, 10 + 16, win.w - 15 - 40, ((row_count < 24 ? row_count : 24) * 16) + 16);
    ...
    icon[0][0] = 128 + (type * 2);
    icon[0][1] = 129 + (type * 2);
    icon[1][0] = 128 + 16 + (type * 2);
    icon[1][1] = 129 + 16 + (type * 2);
    ...
    if (button4_ptr != NULL)
    {
        ...
        widget_ptr = EG_Button_Create("4", col, EG_BUTTON_ALIGN_CENTER, button4_ptr, loc);
        (void)EG_Button_SetMyCallback_OnClick(widget_ptr, MsgBox_Button_Press, NULL);
        (void)EG_Window_AddWidget(window_ptr, widget_ptr);
    ...
    if (button3_ptr != NULL)
    {
        ...
        loc.w = (10 * (strlen(button3_ptr) + 1));
        button_count++;

        widget_ptr = EG_Button_Create("3", col, EG_BUTTON_ALIGN_CENTER, button3_ptr, loc);
        (void)EG_Button_SetMyCallback_OnClick(widget_ptr, MsgBox_Button_Press, NULL);
        (void)EG_Window_AddWidget(window_ptr, widget_ptr);
