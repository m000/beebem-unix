...
static BOOL AppendDirToPath(char *path, char *dir, size_t len)
{
     ...
     */
    if (!TurnStringIntoAPath(path, len))
        return FALSE;
...
static BOOL CopyFile(char *source_file, char *dest_file)
{
     ...
     * be a security risk).
     */
    if (source_file == NULL || dest_file == NULL || strlen(source_file) == 0 || strlen(dest_file) == 0 ||
    ...
    {
        pERROR(dL "Unable to open source file [%s].", dR, source_file);
        return FALSE;
    }
    ...
    {
        pERROR(dL "Unable to write to dest file [%s].", dR, dest_file);
        fclose(src_f);
        return FALSE;
    }
...
static BOOL CreateDirectory(char *file)
{
    ...
    errno = 0;
    if (mkdir(file, (mode_t)S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH) == 0)
        return TRUE;
...
char *GetUserConfigPath(char *path, size_t len)
{
    ...
    if (!TestDirectory(path))
    {
        ...
        //#	endif
        EG_Draw_FlushEventQueue();
        EG_MessageBox(screen_ptr, EG_MESSAGEBOX_INFORMATION, "Welcome to BeebEm!", WELCOME_MESSAGE, "OK", NULL, NULL,
                      NULL, 0);
...
static BOOL CreateFileFromMaster(char *source_filepath, char *master_filepath)
{
    ...
    if (TestFile(source_filepath) == FALSE)
    {
        ...
        {
            pERROR(dL "Unable to make copy of master file '%s'.", dR, master_filepath);
            return FALSE;
        }
    }
...
#define NUMBER_OF_FILES 9
    char files[NUMBER_OF_FILES][20] = {"sasi0.dat", "scsi0.dat", "scsi0.dsc", "scsi1.dat", "scsi1.dsc",
                                       "scsi2.dat", "scsi2.dsc", "scsi3.dat", "scsi3.dsc"};
    ...
    for (i = 0; i < NUMBER_OF_FILES; i++)
    {
        ...
        if (!TestFile(dst_filepath))
        {
             ...
             * print warning.
             */
            pINFO(dL "Creating %s (may take a while).", dR, files[i]);
