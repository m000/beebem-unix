...
void BeebKeyUp(int row, int col)
{
    ...
    if ((SysViaKbdState[col][row]) && (row != 0))
        KeysDown--;
...
#ifdef KBDDEBUG
    cout << "DoKbdIntCheck KeysDown=" << KeysDown << "pcr & c=" << (int)(SysVIAState.pcr & 0xc);
    cout << " IC32State & 8=" << (int)(IC32State & 8) << " KBDRow=" << KBDRow << "KBDCol=" << KBDCol;
    cout << " oldIFRflag=" << Oldflag << " Newflag=" << (int)(SysVIAState.ifr & 1) << "\n";
#endif
} /* DoKbdIntCheck */
...
static int KbdOP(void)
{
    ...
    if ((KBDCol > 9) || (KBDRow > 7))
        return (0); /* Key not down if overrange - perhaps we should do something more? */
...
void CMOSWrite(unsigned char CMOSAddr, unsigned char CMOSData)
{
    ...
    if (CMOSAddr > 0xd)
    {
        ...
        if (CMDF != NULL)
        {
            for (CMA = 0xe; CMA < 64; CMA++)
                fputc(CMOSRAM[CMA], CMDF);
            fclose(CMDF);
        }
    }
}
...
unsigned char CMOSRead(unsigned char CMOSAddr)
{
    ...
    if (CMOSAddr < 0xa)
    {
        ...
        if (CMOSAddr == 0x00)
            return (BCD(CurTime->tm_sec));
        if (CMOSAddr == 0x02)
            return (BCD(CurTime->tm_min));
        if (CMOSAddr == 0x04)
            return (BCD(CurTime->tm_hour));
        if (CMOSAddr == 0x06)
            return (BCD((CurTime->tm_wday) + 1));
        if (CMOSAddr == 0x07)
            return (BCD(CurTime->tm_mday));
        if (CMOSAddr == 0x08)
            return (BCD((CurTime->tm_mon) + 1));
        if (CMOSAddr == 0x09)
            return (BCD((CurTime->tm_year) - 10));
    }
    if (CMOSAddr > 0x9 && CMOSAddr < 0xe)
        return (0);
    return (0);
}
