...
void PlayUpTil(double DestTime)
{
    ...
    while (DestTime > OurTime)
    {
        for (bufinc = 0; (bufptr < SoundBufferSize) && ((OurTime + bufinc) < DestTime); bufptr++, bufinc++)
        {
            ...
            if (SpeechEnabled)
                if (MachineType != 3)
                    tmptotal += (SpeechBuf[SpeechPtr++] - 128) * 10;

            // Mix in relay sound here
            if (Relay == 1)
                tmptotal += (RelayOffBuf[RelayPos++] - 127) * 10;
            if (Relay == 2)
                tmptotal += (RelayOnBuf[RelayPos++] - 127) * 10;
            if (TapeSoundEnabled)
            {
                ...
                {
                    tapetotal = (int)(sin(((TapeAudio.Samples * (10 * (1 + TapeAudio.CurrentBit))) * 3.14) / 180) *
                                      (80 + (40 * (1 - TapeAudio.CurrentBit))));
                    ...
                    if (TapeAudio.Samples++ >= 36)
                    {
                        ...
                        if (TapeAudio.BytePos <= 10)
                            TapeAudio.CurrentBit = (TapeAudio.Data & (1 << (10 - TapeAudio.BytePos))) ? 1 : 0;
                    }
                    if (TapeAudio.BytePos > 10)
                    {
                        TapeAudio.ByteCount--;
            ...
            //++
            if (RelayPos >= RelayLen[(int)Relay])
                Relay = 0;
            //<-
        } /* buffer loop */
...
void AdjustSoundCycles(void)
{
    if (!UseHostClock)
        return;
    if ((!DirectSoundEnabled) || (!SoundEnabled))
        return; // it just lags the program if there's no sound
...
void SoundInit()
{
    ...
    if (SoundSampleRate == 22050)
        SoundAutoTriggerTime = 10000;
    if (SoundSampleRate == 11025)
    ...
    SoundCycles = 0;
    SoundTrigger = SoundAutoTriggerTime;
...
void Sound_RegWrite(int value)
{
    ...
    else
    {
        ...
        case 1: /* Tone 3 vol */
            ...
            if ((BeebState76489.ToneVolume[3] == 0) && ((value & 15) != 15))
                ActiveChannel[3] = TRUE;
            if ((BeebState76489.ToneVolume[3] != 0) && ((value & 15) == 15))
        ...
        case 3: /* Tone 2 vol */
            ...
            if ((BeebState76489.ToneVolume[2] == 0) && ((value & 15) != 15))
                ActiveChannel[2] = TRUE;
            if ((BeebState76489.ToneVolume[2] != 0) && ((value & 15) == 15))
        ...
        case 5: /* Tone 1 vol */
            ...
            if ((BeebState76489.ToneVolume[1] == 0) && ((value & 15) != 15))
                ActiveChannel[1] = TRUE;
            if ((BeebState76489.ToneVolume[1] != 0) && ((value & 15) == 15))
        ...
        case 7: /* Noise volume */
            if ((BeebState76489.ToneVolume[0] == 0) && ((value & 15) != 15))
                ActiveChannel[0] = TRUE;
            if ((BeebState76489.ToneVolume[0] != 0) && ((value & 15) == 15))
                ActiveChannel[0] = FALSE;
...
void LoadSoundUEF(FILE *SUEF)
{
    ...
    for (Chan = 1; Chan < 4; Chan++)
    {
        ...
        if (Data != 15)
            ActiveChannel[4 - Chan] = TRUE;
        else
            ActiveChannel[4 - Chan] = FALSE;
    }
...
void SaveSoundUEF(FILE *SUEF)
{
    ...
    Noise = BeebState76489.Noise.Freq | (BeebState76489.Noise.FB << 2);
    fputc(Noise, SUEF);
